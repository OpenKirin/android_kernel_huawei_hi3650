...
const struct req_delay_stage_config req_stage_cfg[REQ_PROC_STAGE_MAX]=
{
	...
	{"REQ_RQ_START",				NULL},
	{"REQ_RQ_COMPLETE",				NULL},
	{"REQ_MQ_RQ_START",				NULL},
	{"REQ_MQ_RQ_COMPLETE",			NULL},
	{"REQ_MQ_REQUEUE",				NULL},
};
...
static void bio_latency_check_timer_expire(unsigned long data)
{
	...
	fix_style_count = snprintf(log,LATENCY_LOG_BUF_SIZE,"io-latency: ");
	bio_disk_name = (bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_part) ? bdevname(bio->bi_bdev, b) : "-";
	snprintf(log+fix_style_count,(LATENCY_LOG_BUF_SIZE-fix_style_count),"[%s]: %s(0x%lx) length = %u, current = %lu %d ios reach the warning throttle ",bio_disk_name,io_type_parse(bio->bi_rw),bio->bi_rw,bio->bi_iter.bi_size,jiffies,atomic_read(&latency_warning_total));/*lint !e529 !e438 !e737 !e732*/
	printk(KERN_EMERG "%s \r\n",log);
