...
static int noc_dump(void *dump_addr, unsigned int size)
{
	...
	for (i = 0; i < reg_num; i++) {
		...
		*ptr = readl_relaxed(noc_dump_reg_list[i].addr + noc_dump_reg_list[i].offset);
		ptr++;
    }

	uPERI_INT0_STAT = readl_relaxed(SOC_PMCTRL_PERI_INT0_STAT_ADDR
			(g_noc_dump_info.noc_device_ptr->pmctrl_base));
	uSCPERSTATUS6   = readl_relaxed(SOC_SCTRL_SCPERSTATUS6_ADDR
			(g_noc_dump_info.noc_device_ptr->sctrl_base));
...
ERR_PROBE_DUMP_END:
	...
	memcpy(dump_addr, pt_dump, ret_size);/*[false alarm]:ret_size alredy checked.*/
