				     ...
				     const char *buf, size_t count)
{
	...
	for (i = 0; i < NUM_OUTSTANDING_CFG; i++) {
		...
		reg1 = readl(base + i * sizeof(unsigned int));
		if (reg1 != reg) {
			printk(KERN_ALERT"pmu config outstanging error 0x%x 0x%x\r\n", reg, reg1);
			spin_unlock(&pmu->lock);
			return -EINVAL;
		}
	}
...
static int cci_extra_pmu_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(crg)) {
		...
		pmu->crg_base = crg_ctrl_base;
		val = readl(pmu->crg_base + CRG_CCI500_CTRL1_OFFSET);
		val |= BIT(CRG_CCI_EXTRA_PMU_ENABLE);
		writel(val, pmu->crg_base + CRG_CCI500_CTRL1_OFFSET);
	}
