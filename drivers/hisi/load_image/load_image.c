...
s32 load_and_verify_dtb_data(TEEC_Session *session)
{
    ...
    const char *image_name = PART_MODEM_DTB;
	...
	if (0 != dt_entry_ptr.vrl_size) {
	    ...
	    ret = bsp_read_bin((const char *)PART_MODEM_DTB, dt_entry_ptr.vrl_offset, dt_entry_ptr.vrl_size, SECBOOT_BUFFER);
	    if (SEC_ERROR == ret) {
	...
	ret = load_data_to_os(session, MODEM_DTB, PART_MODEM_DTB, DDR_MCORE_DTS_ADDR, dt_entry_ptr.dtb_offset, dt_entry_ptr.dtb_size);
    if (SEC_ERROR == ret)
    {
