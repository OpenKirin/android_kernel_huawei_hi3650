...
VOS_UINT32 RTC_TimerCtrlBlkInit(VOS_VOID)
{
    ...
    /*lint -e534*/
    VOS_MemSet_s(&g_stRtcSocTimerInfo, sizeof(VOS_TIMER_SOC_TIMER_INFO_STRU), 0x0, sizeof(VOS_TIMER_SOC_TIMER_INFO_STRU));

    VOS_MemSet_s((VOS_VOID *)g_astRtcSocTimerDebugInfo, sizeof(g_astRtcSocTimerDebugInfo), 0x0, sizeof(g_astRtcSocTimerDebugInfo));
    /*lint +e534*/
...
RTC_TIMER_CONTROL_BLOCK *RTC_TimerCtrlBlkGet(VOS_UINT32 ulFileID, VOS_INT32 usLineNo)
{
    ...
    if( 0 == RTC_TimerIdleCtrlBlkNumber )
    {
        ...
        /*lint -e534*/
        VOS_MemSet_s(pDumpBuffer, VOS_DUMP_MEM_TOTAL_SIZE, 0, VOS_DUMP_MEM_TOTAL_SIZE);
        /*lint +e534*/

        /* 防止拷贝内存越界，取最小值 */
        /*lint -e506 -e534 */
        VOS_MemCpy_s(pDumpBuffer, VOS_DUMP_MEM_TOTAL_SIZE, (VOS_VOID *)g_acRtcTimerCtrlBuf,
    ...
    for(;;)
    {
        ...
        while ( VOS_NULL_PTR != RTC_TimerCtrlBlkexpired )
        {
            ...
            if(VOS_TRUE == g_ulTimerlpm)
            {
                ...
                /*lint -e534*/
                vos_printf("rtc_timer: allocpid %d, timername 0x%x.\n",
