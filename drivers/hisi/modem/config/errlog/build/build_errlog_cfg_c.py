...
def get_old_xml_hash_value():
    ...
    log.print_green_text("old xml hash value is:"+str_hash_valude)
...
def calc_xml_file_hash():
    ...
    log.print_normal_text("new xml hash value is:"+hash)
...
if __name__ == "__main__":
    ...
    log.print_normal_text(time.strftime("%Y-%m-%d %H:%M:%S ", time.localtime()))
    errcode = 512
    if(str_old_hash!= str_new_hash):
        errcode = 512
        log.print_red_text("xml is changed,pls commit errlog_cfg.c and xml !")
    else:
        errcode = 0
        log.print_normal_text("xml is not changed ")
    ...
    if errcode == 0:
         log.print_green_text("Everything is done!")
    else :
