...
void socp_set_dstchan_mode(u32 chan, u32 mode)
{
    ...
    socp_printf("success set chan %d, mode %d.\n", chan, mode);
}
...
void socp_get_dstchan_mode(u32 chan)
{
    ...
    socp_printf("success get chan %d, ModeState 0x%08x.\n", chan, ModeState);
}
...
SOCP_ST_CASE_S g_stCase[31] =
{
    {
        // 0
        0, 1, SOCP_CODER_DEST_CHAN_6, 0, SOCP_DATA_TYPE_0, SOCP_ENCSRC_CHNMODE_CTSPACKET, SOCP_CHAN_PRIORITY_0,
...
/* SOCP_LLT_ENCODE_029_START 的辅助测试项:打印目的buffer读写指针 */
void socp_encode_task029_printf_test(void)
{
    int chan;
    u32 pointer[4] = {0};

    chan = (g_stCase[29].DstCh)&0xff;

    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFWPTR(chan), pointer[0]);
    
    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFRPTR(chan), pointer[1]);
    
    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFADDR(chan), pointer[2]);
    
    socp_printf("w:0x%08x, r:0x%08x, addr:0x%08x.\n" ,pointer[0], pointer[1], pointer[2]);
}

/* SOCP_LLT_ENCODE_029_START 的辅助测试项:设置目的buffer读写指针相等 */
void socp_encode_task029_set_wr_ptr_test(void)
{
    int chan;
    u32 pointer[4] = {0};
    ...
    SOCP_REG_WRITE(SOCP_REG_ENCDEST_BUFRPTR(chan), pointer[0]);
...
/* SOCP_LLT_ENCODE_029_START 的辅助测试项:继续编码数据 */
BSP_S32 socp_encode_task029_write_test(void)
{
    SOCP_ST_CASE_S *pCase = (SOCP_ST_CASE_S*)&g_stCase[29];
    int cnt = pCase->u32DataCnt;
    int len = pCase->u32DataLen + 8;
    BSP_U8* packet;
    SOCP_BUFFER_RW_STRU WtBuff;
...
BSP_S32 socp_encode_task029(BSP_VOID *param)
{
    SOCP_ST_CASE_S *pCase = (SOCP_ST_CASE_S*)param;
    int cnt = pCase->u32DataCnt;
    int len = pCase->u32DataLen + 8;
    ...
    while(cnt)
    {
        ...
        if(20 == cnt)
        {
            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFWPTR(chan), pointer[0][0]);

            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFRPTR(chan), pointer[0][1]);

            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFADDR(chan), pointer[0][2]);
            
            /* 切换模式 */
            socp_set_dstchan_mode((pCase->DstCh & 0xf), 1);
            
            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFWPTR(chan), pointer[1][0]);

            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFRPTR(chan), pointer[1][1]);

            SOCP_REG_READ(SOCP_REG_ENCDEST_BUFADDR(chan), pointer[1][2]);
...
//        msleep(1);
    ...
    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFWPTR(chan), pointer[2][0]);
    
    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFRPTR(chan), pointer[2][1]);
    
    SOCP_REG_READ(SOCP_REG_ENCDEST_BUFADDR(chan), pointer[2][2]);
    
    socp_printf("0: 0x%08x, 0x%08x, 0x%08x.\n 1: 0x%08x, 0x%08x, 0x%08x.\n 2: 0x%08x, 0x%08x, 0x%08x.\n"
        ,pointer[0][0], pointer[0][1], pointer[0][2]
        ,pointer[1][0], pointer[1][1], pointer[1][2]
        ,pointer[2][0], pointer[2][1], pointer[2][2]);
...
/* 编码模式测试，启动编码后切换到循环模式 */
/* 测试流程 */
/* SOCP_LLT_ENCODE_029_START            启动测试，且设置为循环模式 */
/* socp_encode_task029_printf_test      打印读写指针(读写指针不等) */
/* socp_encode_task029_write_test       再次启动编码 */
/* socp_encode_task029_printf_test      打印读写指针(读指针比写指针大一) */
/* socp_encode_task029_write_test       再次启动编码 */
/* socp_encode_task029_printf_test      打印读写指针(读指针比写指针大一) */
/* socp_encode_task029_set_wr_ptr_test  使读写指针相等 */
/* socp_set_dstchan_mode 6 0            设置为阻塞模式 */
/* socp_encode_task029_write_test       再次启动编码 */
/* socp_encode_task029_printf_test      打印读写指针(读写指针相等) */
/* socp_set_dstchan_mode 6 1            设置为循环模式 */
/* socp_encode_task029_write_test       再次启动编码 */
/* socp_encode_task029_printf_test      打印读写指针(读写指针不等) */
/* socp_encode_task029_write_test       再次启动编码 */
/* socp_encode_task029_printf_test      打印读写指针(读指针比写指针大一) */
/* socp_encode_task029_set_wr_ptr_test  使读写指针相等 */
/* socp_set_dstchan_mode 6 0            设置为阻塞模式 */
/* SOCP_LLT_ENCODE_029_STOP             结束测试 */

BSP_S32 SOCP_LLT_ENCODE_029_START(void)
{
    char taskName[30] = {0};

    g_ul_rev_pkt_flag = false;

    CHECK_RET(SocpStInitChannel(&g_stCase[29]), BSP_OK);

    sprintf(taskName,"socpTest%02d",29);

    g_stCase[29].TaskId = kthread_run(socp_encode_task029, (BSP_VOID*)(&g_stCase[29]), taskName);
    if(IS_ERR(g_stCase[29].TaskId))
    {
		socp_printf("create kthread socp_enccode_task001 failed!\n");
        return BSP_ERROR;
    }
...
/* 编码模式测试, 关闭任务 */
BSP_S32 SOCP_LLT_ENCODE_029_STOP(void)
{
    /*停止通道*/
    CHECK_RET(bsp_socp_stop(g_stCase[29].SrcCh), BSP_OK);
    CHECK_RET(bsp_socp_free_channel(g_stCase[29].SrcCh), BSP_OK);

    CHECK_RET(socp_soft_free_encdst_chan(g_stCase[29].DstCh), BSP_OK);

    //socp_free(g_pSocpBuf[--g_SocpBufCnt]);      /* 任务中打包所用内存 */

    TM_socp_free(g_pSocpBuf[--g_SocpBufCnt]);      /* 编码源块数据内存*/
    if(g_stCase[29].eMode == SOCP_ENCSRC_CHNMODE_LIST)
    {
        TM_socp_free(g_pSocpBuf[--g_SocpBufCnt]);  /* 编码RD buffer内存 */
    }
    /*释放申请的内存*/
    if(g_stCase[29].bDstSet)
    {
        TM_socp_free(g_pSocpBuf[--g_SocpBufCnt]);  /* 编码目的通道内存 */
        g_stCase[29].bDstSet = FALSE;
    }

    /*释放任务*/
    if(g_stCase[29].TaskId)
    {
    	//kthread_stop(g_stCase[0].TaskId);
        g_stCase[29].TaskId = NULL;
    }
