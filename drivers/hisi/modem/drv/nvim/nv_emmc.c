...
u32 nv_blk_read(struct nv_emmc_file_header_stru* fd,FSZ off,u32 len,u8* ptr)
{
    ...
    ret = (u32)bsp_blk_read((char*)fd->name,offset,ptr,len,NULL);
...
u32 nv_emmc_area_name_init(void)
{
    ...
    int block;
    /*升级分区*/
    label = PARTITION_MODEM_NVDLOAD_NAME;
    strncpy((char*)g_emmc_area_name[NV_FILE_DLOAD], label, strlen(label));

    /*出厂分区*/
    label = PARTITION_MODEM_NVDEFAULT_NAME;
    strncpy((char*)g_emmc_area_name[NV_FILE_DEFAULT], label, strlen(label));

    /*备份分区*/
    label = PARTITION_MODEM_NVBACKUP_NAME;
    strncpy((char*)g_emmc_area_name[NV_FILE_BACKUP], label, strlen(label));

    /*系统分区*/
    label = PARTITION_MODEM_NVSYS_NAME;
    strncpy((char*)g_emmc_area_name[NV_FILE_SYS_NV], label, strlen(label));

    block = NV_FILE_DLOAD;
    strncpy(g_nv_file[block].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_XNV_CARD_1].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_XNV_CARD_2].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_XNV_CARD_3].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));

    strncpy(g_nv_file[NV_FILE_CUST_CARD_1].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_CUST_CARD_2].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_CUST_CARD_3].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));

    strncpy(g_nv_file[NV_FILE_XNV_MAP_CARD_1].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_XNV_MAP_CARD_2].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
    strncpy(g_nv_file[NV_FILE_XNV_MAP_CARD_3].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));

    block = NV_FILE_BACKUP;
    strncpy(g_nv_file[block].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));

    block = NV_FILE_SYS_NV;
    strncpy(g_nv_file[block].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));

    block = NV_FILE_DEFAULT;
    strncpy(g_nv_file[block].name, g_emmc_area_name[block], strlen(g_emmc_area_name[block]));
...
s32 nv_emmc_read(u8* ptr, u32 size, u32 count, FILE* fp)
{
    ...
    ret = nv_blk_read(fd,(fd->off+fd->seek),real_size,ptr);/*读取注意文件seek位置*/
    if(ret != NAND_OK)
    {
...
s32 nv_emmc_write(u8* ptr, u32 size, u32 count, FILE* fp)
{
    ...
    ret = (u32)nv_blk_write(fd,(fd->off+fd->seek),len,ptr);
    if(ret)
    {
...
u32 nv_read_dload_file(u8* ptr, u32 size, u32 count,u32 offset)
{
    ...
    ret = (u32)bsp_blk_read(g_nv_file[NV_FILE_DLOAD].name, offset, ptr, len, NULL);
    if(ret != NAND_OK)
    {
        printf("\n[%s]\n read nand fail\n",__func__);
        return ret;
...
void show_emmc_info(void)
{
    ...
    printf("\n******************img info*********************\n");
    ret = bsp_blk_read((char*)NV_SYS_SEC_NAME,0,&ctrl_info,sizeof(ctrl_info),NULL);
    if(ret)
        return;
    /* coverity[uninit_use] */
    if(ctrl_info.magicnum != NV_CTRL_FILE_MAGIC_NUM)
    ...
    ret = bsp_blk_read((char*)NV_SYS_SEC_NAME,sizeof(ctrl_info),file_info,ctrl_info.file_size,NULL);
    if(ret)
    {
