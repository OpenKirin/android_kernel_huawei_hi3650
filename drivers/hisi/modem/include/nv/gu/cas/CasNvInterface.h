...
#define NV_ITEM_CAS_1X_TCH_MEAS_FILTER_INFO_SIZE    (8)         /* 对应NV_Item_CAS_1X_NVIM_TCH_MEAS_FILTER */
...
typedef struct
{
    VOS_UINT8                           ucWaitCprocRfIndTimerLen;               /* SRLTE:时隙模式下收到NO_RF_IND,等CPROC的RF_IND的定时器时长 */
    VOS_UINT8                           ucWaitSrlteSysSyncTimerAddLen;          /* SRLTE:搜网流程中定时器延长的长度 */
...
typedef struct
{
    ...
    VOS_INT8                            sCandSetMeasReportThod;                 /* 候选集中上报PSMM的门限, 单位 1db, 候选集中测量值大于此门限值时上报PSMM */

    VOS_UINT8                           ucNghbrExtraExccedTime;                 /* TCH邻区集测量额外需要超过门限次数，才触发上报 */
    VOS_UINT8                           ucCandExtraExccedTime;                  /* TCH候选集测量额外需要超过门限次数，才触发上报 */
...
typedef struct
{
    ...
    VOS_INT8                            cDynamicThreshAdjustValue;              /* 动态门限调整参数，0.125db为单位，默认值为12 */
