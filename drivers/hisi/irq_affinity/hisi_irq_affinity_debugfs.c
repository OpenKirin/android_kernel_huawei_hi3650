...
#define IRQAFF_DEBUGFS_WRITE_DEBUGFS_CMD_SIZE 128
...
{
	char debugfs_cmd[IRQAFF_DEBUGFS_WRITE_DEBUGFS_CMD_SIZE];
	char *_cmd = NULL;
	char *_tmp = NULL;
	unsigned int irq;
	unsigned int cpu;
	...
	buf_size = min(cnt - 1, (size_t)(sizeof(debugfs_cmd) - 1));
	if (copy_from_user(debugfs_cmd, ubuf, buf_size)) {
		cnt = -EINVAL;
		goto out;
	}

	debugfs_cmd[buf_size] = '\0';
	_cmd = debugfs_cmd;
	pr_debug("%s: [cmd: %s[cnt: %d]]\n", MODULE_NAME, debugfs_cmd,
		 (int)cnt);
