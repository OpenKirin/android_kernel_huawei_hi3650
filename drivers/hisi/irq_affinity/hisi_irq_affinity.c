...
int hisi_irqaffinity_register(unsigned int irq, int cpu)
{
	...
	if (cpu >= nr_cpu_ids) {
		pr_err(MODULE_NAME " error cpu num %d\n", cpu);
		ret = -EINVAL;
		goto out;
	}
...
void hisi_irqaffinity_unregister(unsigned int irq)
{
	...
	for (cpu = 0; cpu < nr_cpu_ids; cpu++) {
		...
		if (gotten) {
			pr_info
			    ("Unregister irq-%u affinity on cpu-%d(->cpu-0)\n",
			     irq, cpu);
	...
	if (!gotten)
		pr_warn("Fail to unregister irq affinity,irq-%u\n", irq);
}
