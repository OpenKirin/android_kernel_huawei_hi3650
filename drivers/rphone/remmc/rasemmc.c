...
static int rasprobe_handler_entry(mmc_request_done)
	...
	fault = fault_include(FAULT_READ_DELAY_NONBLOCK);
	if (!fault)
		fault = fault_include(FAULT_WRITE_DELAY_NONBLOCK);
	if (fault) {
		if ((read && (fault->type == FAULT_READ_DELAY_NONBLOCK)) ||
		(write && (fault->type == FAULT_WRITE_DELAY_NONBLOCK))) {
