...
dhd_bus_schedule_queue(struct dhd_bus  *bus, uint16 flow_id, bool txs)
{
	...
	if (flow_id >= bus->max_sub_queues) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulTxErr, ulTxFlowidInvalidErr);
#endif
...

	{
		...
		if (flow_ring_node->status != FLOW_RING_STATUS_OPEN) {
				...
				/* 异常统计 */
				MAC_STREAM_COUNT_PAR2(ulTxErr, ulTxFlowringStateNotOpenErr);
#endif
...
dhd_bus_txdata(struct dhd_bus *bus, void *txp, uint8 ifidx)
{
...
#ifdef CONFIG_NET_STREAM
	MAC_AND_WIFI_STAT_STRU *pstMacMesgTrackStat = NULL;
	unsigned int uiNetStreamFlag = ((struct sk_buff *)(txp))->uiNetStreamFlag;
	unsigned int uiNetStreamIndex = ((struct sk_buff *)(txp))->uiNetStreamIndex;
#endif
	...
	if (!bus->txmode_push) {
		...
		if (!bus->dhd->flowid_allocator) {
			...
			/* 异常统计 */
			MAC_STREAM_COUNT_PAR2(ulTxErr, ulTxFlowringNotIntitedErr);

			/* 流跟踪 */
			MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulTxFlowringNotIntitedErr);
#endif
		...
		if ((flowid >= bus->dhd->num_flow_rings) ||
			...
			/* 异常统计 */
			MAC_STREAM_COUNT_PAR2(ulTxErr, ulTxFlowidOrRingNodeStateErr);

			/* 流跟踪 */
			MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulTxFlowidOrRingNodeStateErr);
#endif
		...
		if (status != FLOW_RING_STATUS_OPEN) {
			...
			if (txp_pend) {
				...
				/* 异常统计 */
				MAC_STREAM_COUNT_PAR2(ulTxErr, ulEqPktStaTxPendErr);

				/* 流跟踪 */
				MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulEqPktStaTxPendErr);
#endif
		...
		if (txp_pend) {
			...
			if ((ret = dhd_flow_queue_enqueue(bus->dhd, queue, txp_pend)) != BCME_OK) {
				...
				/* 异常统计 */
				MAC_STREAM_COUNT_PAR2(ulTxErr, ulEqPktTxPendErr);

				/* 流跟踪 */
				MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulEqPktTxPendErr);
#endif
...
dhd_bus_dpc(struct dhd_bus *bus)
{
	...
	if (bus->dhd->busstate == DHD_BUS_DOWN) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxDpcBusDownErr);
#endif
...
dhdpcie_bus_process_mailbox_intr(dhd_bus_t *bus, uint32 intstatus)
{
	...
	if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6) ||
		...
		if (bus->dhd->busstate == DHD_BUS_SUSPEND) {
			...
			/* 异常统计 */
			MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxDpcBusSuspendErr);
#endif
