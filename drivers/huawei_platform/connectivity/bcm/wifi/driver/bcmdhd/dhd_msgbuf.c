...
dhd_prot_process_msgbuf_rxcpl(dhd_pub_t *dhd)
{
	...
	while (TRUE) {
		...
		if (src_addr == NULL) {
			...
			/* 异常统计 */
			MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxSrcAddrNullErr);
#endif
...
dhd_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint8* buf, uint16 len)
{
	...
	if (msglen == 0) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxMsgLenErr);
#endif
...
#ifdef CONFIG_NET_STREAM
			/* 异常统计 */
			MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxPktLenErr);
#endif
...
dhd_prot_rxcmplt_process(dhd_pub_t *dhd, void* buf, uint16 msglen)
{
	...
	if (!pkt) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulRxErr, ulRxPktBufNullErr);
#endif
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	if (pktid == DHD_PKTID_INVALID) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulTxErr, ulTxPktIdInvalidErr);


		/* 流跟踪 */
		MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulTxPktIdInvalidErr);
	...
	if (txdesc == NULL) {
		...
		/* 异常统计 */
		MAC_STREAM_COUNT_PAR2(ulTxErr, ulHostTxBufNotAvailableErr);


		/* 流跟踪 */
		MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxErr, ulHostTxBufNotAvailableErr);
#endif
...
#ifdef CONFIG_NET_STREAM
	/* 正常统计 */
	MAC_STREAM_COUNT_PAR2(ulTxProtDataOk, ulTxOk);

	/* 流跟踪 */
	MAC_STREAM_TRACK_PAR2(uiNetStreamFlag, uiNetStreamIndex, pstMacMesgTrackStat, ulTxProtDataOk, ulTxOk);
#endif
