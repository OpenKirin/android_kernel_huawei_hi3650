...
static int mcu_power_log_process(const pkt_header_t *head)
{
	...
	hwlog_info("last suspend iomcu idle time is %d , active apps is  0x%x \n",i_power_status.idle_time,i_power_status.active_app_during_suspend);
	return 0;
}
...
module_init(iomcu_power_info_init);
