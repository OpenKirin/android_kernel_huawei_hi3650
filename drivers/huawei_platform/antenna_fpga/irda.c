			   ...
			   loff_t *f_pos)
{
	...
	hwlog_info("%s: %d, count=%u\n", __FUNCTION__, __LINE__, count);//rec
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	if (missing == 0) {
		print_hex_dump(KERN_WARNING, NULL, DUMP_PREFIX_OFFSET, 16, 1,
			       g_indata, count, false);

		status = ice40_set_cs_callback(NORMAL_RATE, g_spi_fpga_cs_gpio);
		hwlog_info("%s: %d, status=%u\n", __FUNCTION__, __LINE__,status);//rec
			  ...
			  struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	...
	hwlog_info("%s - enter.\n", __func__);
	hwlog_info("%s - u_xfers=%p, n_xfers=%u\n", __func__, u_xfers, n_xfers);
	...
	for (n = n_xfers, k_tmp = k_xfers, u_tmp = u_xfers;
		...
		if (u_tmp->tx_buf) {
			k_tmp->tx_buf = buf;
			print_hex_dump(KERN_WARNING, "tx: ", DUMP_PREFIX_OFFSET,
				       16, 1, u_tmp->tx_buf, u_tmp->len, false);//rec
		...
		dev_warn(&spidev->spi->dev,
			 ...
			 u_tmp->delay_usecs,
			 u_tmp->speed_hz ? : spidev->spi->max_speed_hz);//rec
#endif
		status = ice40_set_cs_callback(NORMAL_RATE, g_spi_fpga_cs_gpio);
	...
	for (n = n_xfers, u_tmp = u_xfers; n; n--, u_tmp++) {
		if (u_tmp->rx_buf) {
			...
			print_hex_dump(KERN_WARNING, "rx: ", DUMP_PREFIX_OFFSET,
				       16, 1, u_tmp->rx_buf, u_tmp->len, false);//rec
		}
		buf += u_tmp->len;
	}
	status = total;
...
static long spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	hwlog_info("%s - spidev is ioctl, arg=%lu.\n", __func__, arg);//rec
	...
	case SPI_IOC_RD_MODE:
		hwlog_info("%s - SPI_IOC_RD_MODE. 0x%d\n", __func__,
			   spidev->spi->mode & SPI_MODE_MASK);//rec
		retval =
		    __put_user(spidev->spi->mode & SPI_MODE_MASK,
			       (__u8 __user *) arg);
		break;
	case SPI_IOC_RD_LSB_FIRST:
		hwlog_info("%s - SPI_IOC_RD_LSB_FIRST. 0x%d\n", __func__,
			   (spidev->spi->mode & SPI_LSB_FIRST));//rec
		retval =
		    __put_user((spidev->spi->mode & SPI_LSB_FIRST) ? 1 : 0,
			       (__u8 __user *) arg);
		break;
	case SPI_IOC_RD_BITS_PER_WORD:
		hwlog_info("%s - SPI_IOC_RD_BITS_PER_WORD. 0x%d\n", __func__,
			   spidev->spi->bits_per_word);//rec
		retval =
		    __put_user(spidev->spi->bits_per_word, (__u8 __user *) arg);
		break;
	case SPI_IOC_RD_MAX_SPEED_HZ:
		hwlog_info("%s - SPI_IOC_RD_MAX_SPEED_HZ. %d\n", __func__,
			   spidev->spi->max_speed_hz);
		retval =
		    __put_user(spidev->spi->max_speed_hz, (__u32 __user *) arg);
		break;
...
static long spidev_compat_ioctl(struct file *filp, unsigned int cmd,
				unsigned long arg)
{
	hwlog_info("%s - spidev is compat ioctl, arg=%lu.\n", __func__, arg);//rec
...
int __init spidev_init(void)
{
	...
	status =
	    alloc_chrdev_region(&dev, spidev_minor, spi_nr_devs, "spidev_fpga");
	spidev_major = MAJOR(dev);
	hwlog_info("%s: %d, spidev_major=%u\n", __FUNCTION__, __LINE__,
		   spidev_major);
