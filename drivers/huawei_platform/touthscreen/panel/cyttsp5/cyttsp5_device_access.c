...
static int cyttsp5_ret_scan_data_cmd_(struct device *dev, u16 read_offset,
				      u16 read_count, u8 data_id, u8 *response,
				      u8 *config, u16 *actual_read_len,
				      u8 *return_buf)
{
	int rc;

	rc = cmd->nonhid_cmd->retrieve_panel_scan(dev, 0, read_offset,
						  read_count, data_id, response,
					   ...
					   u16 *actual_read_len, u8 *data)
{
	...
	rc = cmd->nonhid_cmd->get_data_structure(dev, 0, read_offset,
						 read_length, data_id, status,
						 data_format, actual_read_len,
						 data);
	if (rc < 0)
				     ...
				     u8 *results_available)
{
	...
	rc = cmd->nonhid_cmd->run_selftest(dev, 0, test_id,
					   ...
					   summary_result, results_available);
	if (rc < 0)
				       ...
				       char *buf)
{
	...
	rc = cyttsp5_ret_scan_data_cmd_(dev, 0, 0xFFFF,
					dad->panel_scan_data_id, dad->ic_buf,
				     ...
				     struct cyttsp5_attribute *attr, char *buf)
{
	...
	rc = cyttsp5_get_data_structure_cmd_(dev, 0, PIP_CMD_MAX_LENGTH,
					     dad->get_idac_data_id, &cmd_status,
					     &data_format, &act_length,
					     &dad->ic_buf[5]);
	if (rc < 0) {
...
#ifdef TTHE_TUNER_SUPPORT
	dad->tthe_get_panel_data_debugfs =
	    debugfs_create_file(CYTTSP5_TTHE_TUNER_GET_PANEL_DATA_FILE_NAME,
...
static void cyttsp5_device_access_user_command_work_func(struct work_struct
							 *work)
{
...
static int __init cyttsp5_device_access_init(void)
{
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		TS_LOG_INFO
		    ("%s: Registering device access module for core_id: %s\n",
		     __func__, core_ids[i]);
...
fail_unregister_devices:
	for (i--; i >= 0; i--) {
		...
		TS_LOG_INFO
		    ("%s: Unregistering device access module for core_id: %s\n",
		     __func__, core_ids[i]);
	}
	return rc;
}
