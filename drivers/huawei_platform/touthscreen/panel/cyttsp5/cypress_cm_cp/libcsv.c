...
int
csv_fini(struct csv_parser *p, void (*cb1) (void *, size_t, void *),
...
void
csv_set_realloc_func(struct csv_parser *p,
		     void *(*f) (const void *, size_t, gfp_t))
{
...
void csv_set_free_func(struct csv_parser *p, void (*f) (const void *))
{
...
static int csv_increase_buffer(struct csv_parser *p)
{
	...
	while ((vp =
		p->realloc_func((const void *)p->entry_buf,
				p->entry_size + to_add, GFP_KERNEL)) == NULL) {
		to_add /= 2;
		if (!to_add) {
			p->status = CSV_ENOMEM;
			return -1;
...
size_t
csv_parse(struct csv_parser *p, const void *s, size_t len,
	  void (*cb1) (void *, size_t, void *), void (*cb2) (int c, void *),
	  void *data)
{
	...
	if (!p->entry_buf && pos < len) {
		...
		if (csv_increase_buffer(p) != 0) {
			p->quoted = quoted, p->pstate = pstate, p->spaces =
			    spaces, p->entry_pos = entry_pos;
			return pos;
		}
	}
	...
	while (pos < len) {
		...
		if (entry_pos ==
		    ((p->options & CSV_APPEND_NULL) ? p->entry_size -
		     1 : p->entry_size)) {
			if (csv_increase_buffer(p) != 0) {
				p->quoted = quoted, p->pstate =
				    pstate, p->spaces = spaces, p->entry_pos =
				    entry_pos;
				return pos;
			}
		}
