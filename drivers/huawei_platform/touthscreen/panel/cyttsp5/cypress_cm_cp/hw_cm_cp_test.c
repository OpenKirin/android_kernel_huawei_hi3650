...
static struct cyttsp5_panel_info hw_panel_info[] = {
	...
	{"BTV","CS448", 0,"ofilm"},
	{"BTV","CS448", 1,"eely"},
	{"berlin","CYTT21403", 0,"ofilm"},
	{"berlin","CYTT21403", 6,"lens"},
	{"berlin","CYTT21403", 1,"eely"},
	{"prague","CYTT21403", 0,"ofilm"},
	{"prague","CYTT21403", 1,"truly"},
	{"prague","CYTT21403", 2,"eely"},
};
...
static int cyttsp5_get_tp_config_file(struct device *dev, char *parameter_file,
				      char *config_file, char* file_dir)
{
	int error = 0;
	u8 panel_id = 0;
	int length = 0;
	...
	strncat(parameter_file, file_dir, strlen(file_dir));
	...
	TS_LOG_DEBUG("%s, parameters = %s\n", __func__, parameter_file);

	/*get tp config_file    liszt_truly_input_format.csv*/
	strncat(config_file, file_dir, strlen(file_dir));
...
int cyttsp5_cm_cp_test(struct seq_file *m, void *v)
{
	...
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	char parameter_filename_feima3[CONFIG_NAME_LENGTH] = { 0 };
	char config_filename_feima3[CONFIG_NAME_LENGTH] = { 0 };
	char parameter_filename_feima2[CONFIG_NAME_LENGTH] = { 0 };
	char config_filename_feima2[CONFIG_NAME_LENGTH] = { 0 };
	...
	char config_file_dir_feima2[] =
			"/system/vendor/etc/firmware/tp_test_parameters/";
	...
	error = cyttsp5_get_tp_config_file(dev, parameter_filename_feima3,
			config_filename_feima3, config_file_dir_feima3);
	if (error < 0) {
		TS_LOG_ERR("%s, get tp config failed\n", __func__);
		seq_printf(m, "1F;-software_reason");
		goto out;
	}
	error = cyttsp5_get_tp_config_file(dev, parameter_filename_feima2,
			config_filename_feima2, config_file_dir_feima2);
	if (error < 0) {
		TS_LOG_ERR("%s, get tp config failed\n", __func__);
		seq_printf(m, "1F;-software_reason");
		goto out;
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	config_file = filp_open(config_filename_feima3, O_RDONLY, 0);
	if (IS_ERR(config_file)) {
		config_file = filp_open(config_filename_feima2, O_RDONLY, 0);
		if (IS_ERR(config_file)) {
			TS_LOG_ERR("%s,  filp_open error, file name is %s.\n",
					__func__, config_filename_feima2);
			error = -1;
			seq_printf(m, "1F;-software_reason");
			goto exit_open_config_file;
		}
		TS_LOG_INFO("%s,  filp_open %s success.\n", __func__,
				config_filename_feima2);
	} else {
		TS_LOG_INFO("%s,  filp_open %s success.\n", __func__,
			config_filename_feima3);
	}
	...
	if (!cd->pdata->core_pdata->ttda_cap_test_enable) {
		parameter_file = filp_open(parameter_filename_feima3, O_RDONLY, 0);
		if (IS_ERR(parameter_file)) {
			parameter_file = filp_open(parameter_filename_feima2, O_RDONLY, 0);
			if (IS_ERR(parameter_file)) {
				TS_LOG_ERR("%s,  filp_open error, file name is %s.\n",
					__func__, parameter_filename_feima2);
				error = -1;
				seq_printf(m, "1F;-software_reason");
				goto exit_open_parameter_file;
			}
			TS_LOG_INFO("%s,  filp_open %s success.\n", __func__,
				parameter_filename_feima2);
		} else {
			TS_LOG_INFO("%s,  filp_open %s success.\n", __func__,
				parameter_filename_feima3);
		}
	}

	cyttsp5_stop_wd_timer(cd);

	if (cd->mmi_test_gen5 == 1) {
		error = cm_cp_test_run_gen5(NULL, parameter_file, config_file,
						m, vdda, run_cm_test, run_cp_test,
						&cm_test_pass, &cp_test_pass);
	} else if (cd->pdata->core_pdata->ttda_cap_test_enable) {
		seq_printf(m, "1P-");
...
void cyttsp5_mmitest_procfs_create(void)
{
	...
	proc_create("touchscreen/tp_capacitance_data", 0444, NULL,
		    &cyttsp5_cm_cp_proc_fops);
	return;
}
