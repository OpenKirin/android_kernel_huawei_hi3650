...
static int pip_get_sensor_data(struct device *dev, uint8_t *report_buffer,
			       int report_size, uint8_t *response_buffer,
			       int *response_size)
{
	int ret = 0;
	int i = 0;
	...
	if (!report_buffer || !report_size || !response_buffer
	    || !response_size) {
	...
	snprintf(temp_command, PAGE_SIZE, "0x%02X ",
		 report_buffer[report_size - 1]);
...
static int check_and_parse_get_system_info_response(uint8_t *buf,
						    ...
						    *sens_conf_data)
{
	...
	cy_data->fw_revctrl =
	    MERGE16(MERGE8(in->return_data[6], in->return_data[7]),
	...
	sens_conf_data->len_x =
	    MERGE8(in->return_data[30], in->return_data[31]);
	sens_conf_data->len_y =
	    MERGE8(in->return_data[32], in->return_data[33]);
	sens_conf_data->res_x =
	    MERGE8(in->return_data[34], in->return_data[35]);
	sens_conf_data->res_y =
	    MERGE8(in->return_data[36], in->return_data[37]);
	sens_conf_data->max_z =
	    MERGE8(in->return_data[38], in->return_data[39]);
...
static int check_and_parse_get_parameter_response(uint8_t *buf,
						  uint16_t read_length,
						  uint32_t *parameter_value)
{
	...
	if (param_size == 1 && length == 8)
		...
		param_value =
		    MERGE16(MERGE8(in->return_data[2], in->return_data[3]),
...
static int check_and_parse_set_parameter_response(uint8_t *buf,
						  uint16_t read_length,
						    ...
						    uint16_t *data_crc)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_READ_DATA_BLOCK || length < 6)
		return -EINVAL;
						  ...
						  uint8_t data_id)
{
	...
	return prepare_tt_output_report(out, 5,
					COMMAND_RETRIEVE_DATA_STRUCTURE);
}
							    ...
							    uint8_t *data)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_RETRIEVE_DATA_STRUCTURE || length < 6)
		return -EINVAL;
					       ...
					       uint8_t *status)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_SHORT_TEST || length != 7)
		return -EINVAL;
						    ...
						    uint8_t *status)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_CALIBRATE_IDACS || length != 6)
		return -EINVAL;
						       ...
						       uint8_t *status)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_EXECUTE_PANEL_SCAN || length != 6)
		return -EINVAL;
							...
							uint8_t *data)
{
	...
	if (check_and_parse_tt_input_report(in, &length, &command)
	    || command != COMMAND_RETRIEVE_PANEL_SCAN || length < 6)
		return -EINVAL;
...
static int check_and_parse_get_hid_descriptor_response(uint8_t *buf,
						       uint16_t read_length,
						       struct hid_desc
						       *hid_desc)
{
...
static int get_hid_descriptor(uint16_t hid_desc_register,
			      struct hid_desc *hid_desc)
{
	...
	size =
	    prepare_get_hid_descriptor_report(report_buffer, hid_desc_register);

	ret =
	    pip_get_sensor_data(gdev, report_buffer, size, response_buffer,
				&size);
	if (ret < 0) {
	...
	ret =
	    check_and_parse_get_hid_descriptor_response(response_buffer, size,
							hid_desc);
	if (ret < 0) {
...
int pip_read_data_block(uint16_t row_number, uint16_t read_length,
			uint8_t block_id, uint16_t *actual_read_length,
			uint8_t *data)
{
	uint8_t status;
				...
				uint8_t *data_format, uint8_t *data)
{
	...
	size =
	    prepare_retrieve_data_structure_report(report_buffer, read_offset,
						   read_length, data_id);
	...
	if (ret < 0) {
		TS_LOG_ERR
		    ("%s, Unable to send retrieve data structure command!\n",
		     __func__);
		goto exit;
	}
...
int pip_short_test(void)
{
	...
	ret =
	    check_and_parse_short_test_response(response_buffer, size, &status);
	if (ret) {
...
int pip_opens_self_test(void)
{
    ...
    if (ret < 0) {
        TS_LOG_ERR("Unable to send open test command!\n");
        goto exit;
    }
    ret = check_and_parse_open_test_response(response_buffer, size,&status);
    if (ret) {
        TS_LOG_ERR("Check open test fails!\n");
        goto exit;
    }
        ...
        uint8_t *data_format, uint8_t *data)
{
    ...
    if (ret < 0) {
        TS_LOG_ERR("Unable to send retrieve data structure command!\n");
        goto exit;
    }
    ...
    ret = check_and_parse_get_opens_self_test_results_response(response_buffer,
            ...
            data_format, data);
    if (ret) {
        TS_LOG_ERR("Check retrieve data structure fails!\n");
        goto exit;
    }
