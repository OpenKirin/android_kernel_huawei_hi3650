...
struct cyttsp5_core_commands *gcmd;
struct device *gdev;
...
static int parser_set_state(struct parser_data *data, char *field)
{
	...
	case SELF_LOCALPWC_GETING:
		data->pCurFieldAddr =
		    &(data->sensor_info->selflocalPWC.self_gidac_max_rx);
...
static void tp_field_read_callback(void *s, size_t len, void *d)
{
	...
	default:
		...
		if ((data->state == OPENS_GETING)
		    || (data->state == IDAC_GETING)
		    || (data->state == LOCALPWC_GETING)
		    || (data->state == MUTUAL_NOISE_GETING)
		    || (data->state == MUTUAL_RAWDATA_GETING)
		    || (data->state == SELF_NOISE_GETING)
		    || (data->state == SELF_RAWDATA_GETING)
		    || (data->state == SELF_LOCALPWC_GETING)) {
			/*ok*/
		...
		if (ret == 1) {
			TS_LOG_DEBUG
			    ("%s: state is %d, index is %d, data is %d\n",
...
int tp_configuration_get(char *filename,
			 struct cyttsp5_sensor_info_limit *sensor_info)
{
	struct csv_parser p;
	uint8_t buffer[1024];
	...
	while (1) {
		...
		bytes_read =
		    vfs_read(config_file, buffer, sizeof(buffer),
			     &(config_file->f_pos));
...
void mmi_info_limit_print(struct cyttsp5_sensor_info_limit *mmi_info)
{
	...
	TS_LOG_INFO("%s,  opens.ebye_rx_allowed_max data is %d\n", __func__,
		    mmi_info->opens.ebye_rx_allowed_max);
	TS_LOG_INFO("%s,  opens.ebye_rx_allowed_max_num data is %d\n", __func__,
		    mmi_info->opens.ebye_rx_allowed_max_num);
	...
	TS_LOG_INFO("%s,  opens.ebye_tx_allowed_max data is %d\n", __func__,
		    mmi_info->opens.ebye_tx_allowed_max);
	TS_LOG_INFO("%s,  opens.ebye_tx_allowed_max_num data is %d\n", __func__,
		    mmi_info->opens.ebye_tx_allowed_max_num);
...
static int cyttsp5_get_sensor_info(struct device *dev)
{
	...
	TS_LOG_INFO("%s,tx_num = %d, rx_num = %d,hw_mmi_index = %d.\n",
		    __func__, mmi_info_limit.tx_num, mmi_info_limit.rx_num,
		    hw_mmi_index);

	if (mmi_info_limit.tx_num <= 0 || mmi_info_limit.rx_num <= 0) {
		TS_LOG_ERR
		    ("%s,get tx number & rx number by systeninfo error.\n",
		     __func__);
		retval = -EINVAL;
		goto out;
	}
	...
	strncat(config_file_name, TP_CONFIG_FILE_PATH,
		strlen(TP_CONFIG_FILE_PATH));
	...
	for (i = 0; i < length; i++) {
		if (panel_id == hw_panel_info[i].panel_id) {
			retval =
			    strncasecmp(hw_panel_info[i].product_name,
					product_name,
					strlen(hw_panel_info[i].product_name));
			if (retval == 0) {
				retval =
				    strncasecmp(hw_panel_info[i].chip_name,
						chip_name,
						strlen(hw_panel_info[i].
						       chip_name));
				if (retval == 0) {
					strncat(config_file_name,
						hw_panel_info[i].id2str,
						strlen(hw_panel_info[i].
						       id2str));
				}
			}
...
static void cyttsp5_build_test_command(struct device *dev, char *cmd,
				       char *data_id, int data_len)
{
	int i, j;
	int readlength = 0;
	...
	for (i = 0, j = 0; i < data_len; i += CY_READ_DATA_LEN, j++) {
		...
		snprintf(tmp_cmd, PAGE_SIZE, " 0x%02X 0x%02X 0x%02X 0x%02X ",
			 i % 256, i / 256, readlength, 0);
		strncat(tmp_cmd_str, tmp_cmd, sizeof(tmp_cmd_str));
		strncat(tmp_cmd_str, data_id, sizeof(tmp_cmd_str));

		strncpy(dad->read_cmd[j], (const char *)tmp_cmd_str,
			sizeof(dad->read_cmd[j]));
				  ...
				  int data_len, int data_byte, int other)
{
	...
	char *open_test_command =
	    "0x04 0x00 0x07 0x00 0x2F 0x00 0x26 0x03 0x00";
	int tx_num = 0;
	int rx_num = 0;
	int index = 0;
	struct cyttsp5_device_access_data *dad =
	...
	if (data_len <= CY_READ_DATA_LEN) {
		...
		read_times =
		    (tx_num * rx_num + tx_num + 2) / CY_READ_DATA_LEN + 1;
	}
	...
	for (i = 0; i < read_times; i++) {
		...
		for (j = CY_DATA_HEAD_LEN; j < dad->response_length;
		     j += data_byte) {
			index =
			    i * CY_READ_DATA_LEN + (j -
						    CY_DATA_HEAD_LEN) /
			    data_byte;
			if (data_byte == CYTTSP5_DATA_ONE_BYTE) {
				...
				dad->data_buf[index] =
				    (int)get_unaligned_le16(&tmp_buf[j]);
			}
...
static int cyttsp5_get_rawdata(struct device *dev)
{
	...
	struct cyttsp5_device_access_data *dad =
	    cyttsp5_get_device_access_data(dev);
	struct cyttsp5_sensor_mutual_rawdata m_rawdata =
	    mmi_info_limit.mutual_rawdata;
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x00", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
	}
	...
	for (i = 0; i < length; i++) {
		if (dad->data_buf[i] > m_rawdata.max[j]
			...
			TS_LOG_ERR
			    ("%s check is failed. dad->data_buf[%d] is %d, rawdata min[%d]:max[%d]\n",
			     __func__, i, dad->data_buf[i], m_rawdata.min[j],
			     m_rawdata.max[j]);
			check_flag = -1;
			break;
		}
...
static int cyttsp5_get_self_rawdata(struct device *dev)
{
	...
	int length = mmi_info_limit.tx_num + mmi_info_limit.rx_num;
	int j = 0;
	struct cyttsp5_sensor_self_rawdata s_rawdata =
	    mmi_info_limit.self_rawdata;
	int iDiff = CSV_RX_NUM - mmi_info_limit.rx_num;

	ret =
	    cyttsp5_get_SensorData(dev, "0x03", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
	...
	j = 0;
	for (i = 0; i < length; i++) {
		if (dad->data_buf[i] > s_rawdata.max[j]
		    || dad->data_buf[i] < s_rawdata.min[j]) {
			TS_LOG_ERR
			    ("%s check is failed. dad->data_buf[%d] is %d. s_rawdata[%d] min %d- max %d \n",
			     __func__, i, dad->data_buf[i], j, s_rawdata.min[j],
			     s_rawdata.max[j]);
			check_flag = -1;
			break;
		}
...
static int cyttsp5_get_mutual_noise(struct device *dev)
{
	...
	struct cyttsp5_sensor_mutual_noise mut_noise =
	    mmi_info_limit.mutual_noise;
	int j = 0;
	int iDiff = CSV_RX_NUM - mmi_info_limit.rx_num;
	...
	for (i = 0; i < length; i++) {
		if (mut_noise.max[j] < (max_raw[i] - min_raw[i])) {
			TS_LOG_ERR
			    ("%s mutual noise check  failed. raw[%d] is %d-%d, mut_noise.max[%d] is %d \n",
			     __func__, i, min_raw[i], max_raw[i], j,
			     mut_noise.max[j]);
			ret = -1;
			goto exit;
		}
...
static int cyttsp5_get_self_noise(struct device *dev)
{
	...
	for (i = 0; i < length; i++) {
		if (s_noise.max[j] < (max_raw[i] - min_raw[i])) {
			TS_LOG_ERR
			    ("%s self noise check  failed.  raw[%d] is %d-%d, s_noise.max[%d] is %d \n",
			     __func__, i, min_raw[i], max_raw[i], j,
			     s_noise.max[j]);
			ret = -1;
			goto exit;
...
static int cyttsp5_get_LocalPWC(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x00", "0x24",
				   length + (tx_num + 4 - tx_num % 4),
	...
	j = 0;
	for (i = 0; i < length; i++) {
		if (dad->data_buf[iHeadNum + i] > localPWC.max[j]
		    || dad->data_buf[iHeadNum + i] < localPWC.min[j]) {
			TS_LOG_ERR
			    ("%s check is failed. dad->data_buf[%d] is %d, localPWC[%d] max[%d] min[%d]\n",
			     __func__, i, dad->data_buf[iHeadNum + i], j,
			     localPWC.max[j], localPWC.min[j]);
			check_flag = -1;
			break;
		}
		if ((i + 1) % mmi_info_limit.rx_num == 0) {
...
static int cyttsp5_get_GlobalGIDAC(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x00", "0x24", tx_num + 4 - tx_num % 4,
				   CYTTSP5_DATA_ONE_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
	...
	for (i = 0; i < tx_num; i++) {
		if (dad->data_buf[i] > idac.max[i]
		    || dad->data_buf[i] < idac.min[i]) {
			TS_LOG_ERR
			    ("%s check is failed, dad->data_buf[%d] is %d, idac max[%d] min[%d].\n",
			     __func__, i, dad->data_buf[i], idac.max[i],
			     idac.min[i]);
			range_flag = -1;
			goto exit;
		}
	...
	for (i = 0; i < tx_num - 1; i++) {
		if (abs(dad->data_buf[i] - dad->data_buf[i + 1]) >
		    idac.delta[i]) {
			TS_LOG_ERR
			    ("%s uniformity check is failed,  delta data is [%d]%d-%d.\n",
...
static int cyttsp5_get_SelfLocalPWC(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x01", "0x24", tx_num + rx_num + 2,
				   CYTTSP5_DATA_ONE_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
	...
	if ((dad->data_buf[0] > sPWC.self_gidac_max_rx)
	    || (dad->data_buf[0] < sPWC.self_gidac_min_rx)) {
		TS_LOG_ERR
		    ("%s  Self-cap Global iDAC check rx is failed. value is %d, min[%d]-max[%d]\n",
		     __func__, dad->data_buf[0], sPWC.self_gidac_min_rx,
		     sPWC.self_gidac_max_rx);
		rx_flag = -1;
		goto exit;
	}

	if ((dad->data_buf[1] > sPWC.self_gidac_max_tx)
	    || (dad->data_buf[1] < sPWC.self_gidac_min_tx)) {
		TS_LOG_ERR
		    ("%s  Self-cap Global iDAC check tx is failed. value is %d, min[%d]-max[%d]\n",
		     __func__, dad->data_buf[1], sPWC.self_gidac_min_tx,
		     sPWC.self_gidac_max_tx);
		tx_flag = -1;
		goto exit;
	}
	...
	for (i = 0; i < rx_add_tx; i++) {
		if ((dad->data_buf[iHeadNum + i] > sPWC.max[j])
		    || (dad->data_buf[iHeadNum + i] < sPWC.min[j])) {
			if (i < rx_num) {
				TS_LOG_ERR
				    ("%s  Self-cap Local PWC rx %d check is failed. value is %d. sPWC[%d] min %d-max %d.\n",
				     __func__, i, dad->data_buf[iHeadNum + i],
				     j, sPWC.min[j], sPWC.max[j]);
				rx_flag = -1;
			} else {
				TS_LOG_ERR
				    ("%s  Self-cap Local PWC tx %d check is failed. value is %d. sPWC[%d] min %d-max %d.\n",
				     __func__, i, dad->data_buf[iHeadNum + i],
				     j, sPWC.min[j], sPWC.max[j]);
				tx_flag = -1;
			}
	...
	for (i = 0; i < rx_add_tx - 1; i++) {
		...
		if (abs
		    (dad->data_buf[iHeadNum + i] -
		     dad->data_buf[iHeadNum + i + 1]) > sPWC.delta[j]) {
			if (i < rx_num) {
				TS_LOG_ERR
				    ("%s  Self-cap Local PWC delta rx %d check is failed. value is %d-%d. sPWC[%d] delta %d.\n",
				     __func__, i, dad->data_buf[iHeadNum + i],
				     dad->data_buf[iHeadNum + i + 1], j,
				     sPWC.delta[j]);
				rx_flag = -1;
			} else {
				TS_LOG_ERR
				    ("%s  Self-cap Local PWC delta tx %d check is failed. value is %d-%d. sPWC[%d] delta %d.\n",
				     __func__, i, dad->data_buf[iHeadNum + i],
				     dad->data_buf[iHeadNum + i + 1], j,
				     sPWC.delta[j]);
				tx_flag = -1;
...
static int cyttsp5_get_OPENS(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x03", "0x27", length,
				   CYTTSP5_DATA_ONE_BYTE, 1);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
	}
	...
	for (i = 0; i < length; i++) {
		if (dad->data_buf[i] < sen_opens.min[j]) {
			TS_LOG_ERR
			    ("%s Opens MIN LIMITS error. %d data is %d, sen_opens.min[%d] is %d\n",
			     __func__, i, dad->data_buf[i], j,
			     sen_opens.min[j]);
			goto exit;
		}
		/*jump over null data*/
	...
	for (i = 0; i < length - 1; i++) {	/*差值需要考虑*/
		...
		if (abs(opens_diff[i]) > sen_opens.ebye_rx[j]) {
			TS_LOG_ERR
			    ("%s rx uniformity E-By-E check is failed, opens_diff[%d] is %d, sen_opens.ebye_rx[%d] is %d\n",
			     __func__, i, opens_diff[i], j,
			     sen_opens.ebye_rx[j]);
			goto exit;
		}
		...
		if (abs(opens_diff[i]) > rx_allowed_max) {
			...
			if (iCount > rx_allowed_max_num) {
				TS_LOG_ERR
				    ("%s rx allowed max data num check failed, data is %d, allowed num is %d\n",
				     __func__, rx_allowed_max,
				     rx_allowed_max_num);
				goto exit;
			}
		}
	...
	for (i = 0; i < rx_num * (tx_num - 1); i++) {
		...
		if (abs(opens_diff[i]) > sen_opens.ebye_tx[j]) {
			TS_LOG_ERR
			    ("%s tx uniformity E-By-E check is failed, opens_diff[%d] is %d, sen_opens.ebye_tx[%d] is %d.\n",
			     __func__, i, opens_diff[i], j,
			     sen_opens.ebye_tx[j]);
			goto exit;
		}
		...
		if (abs(opens_diff[i]) > tx_allowed_max) {
			...
			if (iCount > tx_allowed_max_num) {
				TS_LOG_ERR
				    ("%s tx allowed max data num check failed, data is %d, allowed num is %d\n",
				     __func__, tx_allowed_max,
				     tx_allowed_max_num);
				goto exit;
			}
		}
...
static int cyttsp5_get_mutual_baseline(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x01", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
...
static int cyttsp5_get_self_baseline(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x04", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
...
static int cyttsp5_get_mutual_diff(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x02", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
...
static int cyttsp5_get_self_diff(struct device *dev)
{
	...
	ret =
	    cyttsp5_get_SensorData(dev, "0x05", "0x2B", length,
				   CYTTSP5_DATA_TWO_BYTE, 0);
	if (ret < 0) {
		TS_LOG_ERR("%s error.\n", __func__);
		goto exit;
...
static int cyttsp5_proc_data_print(struct device *dev, int *data_buf,
				   struct seq_file *m)
{
	int index = 0;
	char *temp = NULL;
	...
	case CYTTSP_MUTUAL_LOCALPWC:
		...
		for (index = tx_num + 4 - tx_num % 4;
		     index < length_long + tx_num + 4 - tx_num % 4; index++) {
			if ((index + 1 - (tx_num + 4 - tx_num % 4)) % rx_num ==
			    0) {
				seq_printf(m, "%4d\n", data_buf[index]);
			} else {
...
int cyttsp5_check_items(struct device *dev, enum cyttsp5_sensor_check item,
			bool reset, int *data_buf)
{
	int retry = 0;
	int check_flag = -1;
	struct cyttsp5_device_access_data *dad =
	...
	if (data_buf != NULL) {
		memcpy(data_buf, dad->data_buf,
		       sizeof(int) * CY_MAX_RAW_DATA_LEN);
	}
...
static void cyttsp5_sensor_check(struct device *dev, int *buf[CYTTSP_CHECK_MAX],
				 int *check_count)
{
	int test_flag = 0;
	int index = 0;
	...
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_GLOBAL_IDAC, 1,
				buf[CYTTSP_GLOBAL_IDAC]);
	CHECK_RESULT_SET(dev, test_flag, "1");

	/*short check*/
	dad->check_index = CYTTSP_SHORT_INFO;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_SHORT_INFO, 0,
				buf[CYTTSP_SHORT_INFO]);
	CHECK_RESULT_SET(dev, test_flag, "2");

	/*Mutual cap check*/
	dad->check_index = CYTTSP_MUTUAL_RAWDATA;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_MUTUAL_RAWDATA, 1,
				buf[CYTTSP_MUTUAL_RAWDATA]);
	CHECK_RESULT_SET(dev, test_flag, "3");

	/*LocalPWC check*/
	dad->check_index = CYTTSP_MUTUAL_LOCALPWC;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_MUTUAL_LOCALPWC, 1,
				buf[CYTTSP_MUTUAL_LOCALPWC]);
	CHECK_RESULT_SET(dev, test_flag, "4");

	/*Opens check*/
	dad->check_index = CYTTSP_OPENS_DATA;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_OPENS_DATA, 0,
				buf[CYTTSP_OPENS_DATA]);
	CHECK_RESULT_SET(dev, test_flag, "5");

	/*Self LocalPWC check*/
	dad->check_index = CYTTSP_SELF_LOCALPWC;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_SELF_LOCALPWC, 1,
				buf[CYTTSP_SELF_LOCALPWC]);
	CHECK_RESULT_SET(dev, test_flag, "6");

	/*Self cap check*/
	dad->check_index = CYTTSP_SELF_RAWDATA;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_SELF_RAWDATA, 1,
				buf[CYTTSP_SELF_RAWDATA]);
	CHECK_RESULT_SET(dev, test_flag, "7");

	/*Mutual noise check*/
	dad->check_index = CYTTSP_MUTUAL_NOISE;
	test_flag =
	    cyttsp5_check_items(dev, CYTTSP_MUTUAL_NOISE, 1,
				buf[CYTTSP_MUTUAL_NOISE]);
...
static void cyttsp5_sensor_data_print(struct device *dev,
				      int *data_buf[CYTTSP_CHECK_MAX],
				      struct seq_file *m)
{
	struct cyttsp5_device_access_data *dad =
	    cyttsp5_get_device_access_data(dev);
...
static int cyttsp5_rawdata_proc_show(struct seq_file *m, void *v)
{
	...
	for (index = 0; index < CYTTSP_CHECK_MAX; index++) {
		tmp_buf[index] =
		    kzalloc(sizeof(int) * CY_MAX_RAW_DATA_LEN, GFP_KERNEL);
...
static void cyttsp5_sensor_check_all(struct device *dev,
				     int *buf[CYTTSP_CHECK_MAX],
				     int *check_count)
{
	int test_flag = 0;
	int index = 0;
	...
	for (index = 0; index < CYTTSP_CHECK_MAX; index++) {
		...
		test_flag =
		    cyttsp5_check_items(dev, dad->check_index, 0, buf[index]);
...
static void cyttsp5_sensor_data_print_all(struct device *dev,
					  int *data_buf[CYTTSP_CHECK_MAX],
					  struct seq_file *m)
{
	int index = 0;
...
static int cyttsp5_data_proc_show(struct seq_file *m, void *v)
{
	int error = 0;
	struct cyttsp5_device_access_data *dad =
	    cyttsp5_get_device_access_data(gdev);
	struct device *dev = gdev;
...
void cyttsp5_procfs_create(void)
{
	...
	proc_create("touchscreen/rawdata", 0444, NULL,
		    &cyttsp5_rawdata_proc_fops);
	proc_create("touchscreen/dumpdata", 0444, NULL,
		    &cyttsp5_sensor_data_proc_fops);
