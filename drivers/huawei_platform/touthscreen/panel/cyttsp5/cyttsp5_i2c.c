...
struct dsm_client *tp_cyp_dclient;
...
ssize_t cyttsp5_dsm_record_basic_err_info(struct device *dev)
{
	...
	int vci_value = -1, vci_status = -1, vddio_value = -1, vddio_status =
	    -1;
	...
	if (VCI_LDO_CTRL_MODE == power_ctrl->vci_power_type) {
		...
		snprintf(vci_power_info, DSMINFO_LEN_MAX,
			 "[vci   power] type:ldo, volt:%d, status:%d",
			 vci_value, vci_status);
		...
		snprintf(vci_power_info, DSMINFO_LEN_MAX,
			 "[vci   power] type:gpio, num:%d, status:%d",
			 vci_value, vci_status);

	} else {
		snprintf(vci_power_info, DSMINFO_LEN_MAX,
			 "[vci   power] type:unknown");
	}
	...
	if (VDDIO_LDO_CTRL_MODE == power_ctrl->vddio_power_type) {
		...
		snprintf(vddio_power_info, DSMINFO_LEN_MAX,
			 "[vddio power] type:ldo, volt:%d, status:%d",
			 vddio_value, vddio_status);
		...
		snprintf(vddio_power_info, DSMINFO_LEN_MAX,
			 "[vddio power] type:gpio, num:%3d, status:%d",
			 vddio_value, vddio_status);

	} else {
		snprintf(vddio_power_info, DSMINFO_LEN_MAX,
			 "[vddio power] type:unknown");
	}

	dsm_client_record(tp_cyp_dclient, "%s\n"
			  "%s\n"
			  "[irq    gpio] num: %d, irq   gpio status:%d, irq depth: %d(0:enable)\n"
			  "[reset  gpio] num:%d, reset gpio status:%d\n",
			  vci_power_info,
			  vddio_power_info,
			  g_tp_dsm_info.irq_gpio,
			  gpio_get_value(g_tp_dsm_info.irq_gpio),
			  IS_ERR_OR_NULL(desc) ? -1 : desc->depth,
			  g_tp_dsm_info.rst_gpio,
			  gpio_get_value(g_tp_dsm_info.rst_gpio));
...
error_alloc_data:
	dsm_client_record(tp_cyp_dclient,
			  ...
			  gpio_get_value(g_tp_dsm_info.rst_gpio));
exit:
	TS_LOG_ERR("%s: [irq gpio] num:%3d, irq gpio status:%d,irq depth: %d(0:enable)\n",
		   __func__, g_tp_dsm_info.irq_gpio,
		   gpio_get_value(g_tp_dsm_info.irq_gpio),IS_ERR_OR_NULL(desc) ? -1 : desc->depth);
	TS_LOG_ERR("%s: [reset gpio] num:%3d, reset gpio status:%d\n",
		   __func__, g_tp_dsm_info.rst_gpio,
		   gpio_get_value(g_tp_dsm_info.rst_gpio));
	if(vci_power_info != NULL){
		kfree(vci_power_info);
		vci_power_info = NULL;
	}
	if(vddio_power_info != NULL){
		kfree(vddio_power_info);
		vddio_power_info = NULL;
	}
...
static int cyttsp5_i2c_read_default_nosize(struct device *dev, u8 *buf,
					   u32 max)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static void cyttsp5_core_init_work(struct work_struct *work);

static struct cyttsp5_init_data *core_init_data = NULL;

int cyttsp5_is_init_thread_enable(void)
{
	if(core_init_data && core_init_data->init_thread_enable) {
		TS_LOG_INFO("%s: init_thread_enable is true\n", __func__);
		return 1;
	} else {
		TS_LOG_INFO("%s: init_thread_enable is false\n", __func__);
		return 0;
	}
}

static cyttsp5_prepare_init_thread(struct i2c_client *client,
								struct device *dev)
{
	int ret = 0;
	u32 value = 0;

	ret = of_property_read_u32(dev->of_node, "cy,init_thread_enable",
								&value);
	if (ret || !value) {
		TS_LOG_INFO("%s,init_thread_enable not set\n", __func__);
		return -ENODEV;
	}

	core_init_data = kzalloc(sizeof(struct cyttsp5_init_data),
						GFP_KERNEL);
	if (!core_init_data) {
		TS_LOG_ERR("%s:kzalloc core_init_data fail\n", __func__);
		return -ENODEV;
	}

	core_init_data->client = client;
	core_init_data->dev = &client->dev;
	core_init_data->init_thread_enable = value;
	INIT_WORK(&core_init_data->core_init_work, cyttsp5_core_init_work);
	schedule_work(&core_init_data->core_init_work);
		return 0;
}
...
/* if support device tree, get pdata from device tree */
	...
	if (match) {
		...
		if (rc < 0) {
			TS_LOG_ERR
			    ("%s: device tree create and get pdata fail, rc = %d.\n",
			     __func__, rc);
			return rc;
		}
	} else {
...
#endif
	rc = cyttsp5_prepare_init_thread(client, &client->dev);
	if (!rc) {
		TS_LOG_INFO("%s:core probe will in thread\n", __func__);
		return 0;
	}
...
static int cyttsp5_i2c_remove(struct i2c_client *client)
{
	...
	/*
	 * Only if init_in_thread not config or init_in_thread succ
	 * need to call cyttsp5_release
	 */
	if (!cyttsp5_is_init_thread_enable()||
		(core_init_data && core_init_data->init_status == CYTTSP5_INIT_SUCC))
		cyttsp5_release(cd);
...
static void cyttsp5_core_init_work(struct work_struct *work)
{
	struct cyttsp5_init_data *init_data;
	int ret = 0;
	init_data = container_of(work, struct cyttsp5_init_data,
						core_init_work);

	TS_LOG_INFO("%s: called\n", __func__);

	ret = cyttsp5_probe(&cyttsp5_i2c_bus_ops, init_data->dev,
					init_data->client->irq, CY_I2C_DATA_SIZE);
	if (ret) {
		init_data->init_status = CYTTSP5_INIT_FAIL;
		TS_LOG_ERR("%s:core probe fail, ret = %d\n", __func__, ret);
		i2c_del_driver(&cyttsp5_i2c_driver);
	} else {
		init_data->init_status = CYTTSP5_INIT_SUCC;
		TS_LOG_INFO("%s: core probe succ\n", __func__);
	}
