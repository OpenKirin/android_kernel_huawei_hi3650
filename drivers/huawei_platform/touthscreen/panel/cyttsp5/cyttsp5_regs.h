...
enum cyttsp5_retrieve_panel_scan_data_id {
	MUTUAL_CAP_RAW_DATA,
	MUTUAL_CAP_BASELINE_DATA,
	MUTUAL_CAP_DIFFERENCE,
	SELF_CAP_RAW_DATA,
	SELF_CAP_BASELINE_DATA,
	SELF_CAP_DIFFERENCE,
};

enum cyttsp5_touch_debug_data_type {
	CY_PANEL_SCAN_DIFFDATA,
	CY_PANEL_SCAN_RAWDATA,
	CY_PANEL_SCAN_BASLINE,
	CY_PANEL_SCAN_SUPPORT_NUM,
	CY_PANEL_SCAN_DEFAULT = 0xFF,
};
...
struct cyttsp5_dege_conf_flag {
	int finger_stop_y[10];
	u16 stop_report_flag;
	u16 must_report_flag;
};
...
enum cypress_edge_thr {
	CY_THR_LEFT_C = 0,   //Left Line-C
	CY_THR_LEFT_E,   //Left Line-E
	CY_Y_MOVE_UP,    //40
	CY_Y_MOVE_DOWN,  //40
	CY_EDGE_X_MAX,   //x max 2560
	CY_EDGE_Y_MAX,   //y max 1600
	CY_THR_RIGHT_C,  //Right Line-C
	CY_THR_RIGHT_E,  //Right Line-E
	CY_THR_MAX,
};
...
struct cyttsp5_mt_function {
	...
	void (*report_slot_liftoff) (struct cyttsp5_mt_data *md,
				     int max_slots);
...
struct cyttsp5_mt_data {
	...
	struct cyttsp5_dege_conf_flag conf_flag;
...
#endif
#define CYTTSP5_INIT_SUCC	1
#define CYTTSP5_INIT_FAIL	0
struct cyttsp5_init_data {
	u32 init_thread_enable;
	struct i2c_client *client;
	struct device *dev;
	struct work_struct core_init_work;
	int init_status;
};
...
struct cyttsp5_core_data {
	...
	u8 last_charger_armor_level;
	bool wake_initiated_by_device;
	bool wait_until_wake;
	bool transient_state;
	u8 panel_id;
	char lcd_panel_name[LCD_PANEL_NAME_MAX_LEN + 1];
...
int _cyttsp5_request_hid_output_get_param(struct device *dev,
					  int protect, u8 param_id,
					  u32 *value);
int _cyttsp5_request_hid_output_set_param(struct device *dev, int protect,
...
static inline struct cyttsp5_device_access_data
    *cyttsp5_get_device_access_data(struct device *dev)
{
	return (struct cyttsp5_device_access_data *)
	    cyttsp5_get_dynamic_data(dev, CY_MODULE_DEVICE_ACCESS);
}
...
#else
static inline int cyttsp5_btn_probe(struct device *dev)
{
	return 0;
}

static inline int cyttsp5_btn_release(struct device *dev)
{
	return 0;
}
#endif
...
#else
static inline int cyttsp5_proximity_probe(struct device *dev)
{
	return 0;
}

static inline int cyttsp5_proximity_release(struct device *dev)
{
	return 0;
}
#endif
...
static inline struct cyttsp5_platform_data *cyttsp5_get_platform_data(struct
								      device
								      *dev)
{
