...
static void cyttsp5_area_para_init( struct cyttsp5_mt_data *md)
{
	md->thr[CY_THR_LEFT_C]  = md->pdata->edge_config_buf[CY_THR_LEFT_C];
	md->thr[CY_THR_LEFT_E]  = md->pdata->edge_config_buf[CY_THR_LEFT_E];
	md->thr[CY_Y_MOVE_UP]   = md->pdata->edge_config_buf[CY_Y_MOVE_UP];
	md->thr[CY_Y_MOVE_DOWN] = md->pdata->edge_config_buf[CY_Y_MOVE_DOWN];
	md->thr[CY_EDGE_X_MAX]  = md->pdata->edge_config_buf[CY_EDGE_X_MAX];
	md->thr[CY_EDGE_Y_MAX]  = md->pdata->edge_config_buf[CY_EDGE_Y_MAX];
	...
	TS_LOG_INFO("%s: lc = %d, le = %d\n", __func__, md->thr[CY_THR_LEFT_C],md->thr[CY_THR_LEFT_E]);
	TS_LOG_INFO("%s: rc = %d, re = %d\n", __func__, md->thr[CY_THR_RIGHT_C],md->thr[CY_THR_RIGHT_E]);
	TS_LOG_INFO("%s: up = %d, down = %d\n", __func__, md->thr[CY_Y_MOVE_UP],md->thr[CY_Y_MOVE_DOWN]);
	TS_LOG_INFO("%s: x_max = %d, y_max = %d\n", __func__, md->thr[CY_EDGE_X_MAX],md->thr[CY_EDGE_Y_MAX]);
}
	...
	*axis &= max - 1;
	...
	TS_LOG_DEBUG
	    ("%s: *axis=%02X(%d) size=%d max=%08X xy_data=%p xy_data[%d]=%02X(%d)\n",
	     __func__, *axis, *axis, size, max, xy_data, next, xy_data[next],
	     xy_data[next]);
}
				  ...
				  struct cyttsp5_touch *touch, u8 *xy_mode)
{
	...
	TS_LOG_DEBUG("%s: time=%X tch_num=%d lo=%d noise=%d counter=%d\n",
		     ...
		     touch->hdr[CY_TCH_LO],
		     touch->hdr[CY_TCH_NOISE], touch->hdr[CY_TCH_COUNTER]);
}
...
static int cyttsp5_mt_process_point(struct cyttsp5_mt_data *md,
				     struct cyttsp5_touch *tch,int index)
{
	struct cyttsp5_sysinfo *si = md->si;
	struct cyttsp5_dege_conf_flag *conf_flag = &md->conf_flag;

	int x = tch->abs[CY_TCH_X];
	int y = tch->abs[CY_TCH_Y];

	int flag_inside_c    = 0;
	int flag_between_c_e = 0;
	int flag_between_e_e = 0;
	int flag_move_up     = 0;
	int flag_move_down   = 0;
	int debug_status     = 0;

	if(md->thr[CY_THR_LEFT_C] == 0){
		TS_LOG_INFO("%s:Line-C limit is zore,keep report.\n", __func__);
		return 0;
	}

	flag_inside_c = ((x < md->thr[CY_THR_LEFT_C])|| (x > md->thr[CY_THR_RIGHT_C])) ? 1 : 0;
	flag_between_c_e = ((x >= md->thr[CY_THR_LEFT_C] && x <= md->thr[CY_THR_LEFT_E]) 
	                 || (x <= md->thr[CY_THR_RIGHT_C] && x >= md->thr[CY_THR_RIGHT_E])) ? 1 : 0;

	flag_between_e_e = (x > md->thr[CY_THR_LEFT_E] && x < md->thr[CY_THR_RIGHT_E])  ? 1 : 0;

	TS_LOG_DEBUG("%s: Finger id = %d x = %d y = %d\n", __func__, index, x,y);
	TS_LOG_DEBUG("%s: flag_inside_c = %d\n", __func__,flag_inside_c);
	TS_LOG_DEBUG("%s: flag_between_c_e = %d\n", __func__,flag_between_c_e);
	TS_LOG_DEBUG("%s: flag_between_e_e = %d\n", __func__,flag_between_e_e);
	TS_LOG_DEBUG("%s: must_report_flag = %d\n", __func__,conf_flag->must_report_flag);
	TS_LOG_DEBUG("%s: stop_report_flag = %d\n", __func__,conf_flag->stop_report_flag);

	//First time point.
	if((conf_flag->stop_report_flag & (1 << index)) == 0 && (conf_flag->must_report_flag & (1 << index)) == 0){
		//First time,inside Line-C,then stop report.
		if(flag_inside_c){
			 //First time,store y of the point.
			 conf_flag->finger_stop_y[index] = y;
			 debug_status = 1;
			 goto out_stop_report;
	     }
	     //First time,overstep Line-C,then report.
	     if(flag_between_c_e || flag_between_e_e){
			 debug_status = 2;
			 goto out_must_report;
	     }
	}

	//Last time is must report,keep report.
	if(conf_flag->must_report_flag & (1 << index)){
		debug_status = 3;
		goto out_must_report;
	}

	//Action is moving up or moving down;
	flag_move_down = ((y > conf_flag->finger_stop_y[index] +  md->thr[CY_Y_MOVE_DOWN])) ? 1 : 0;
	flag_move_up   = ((y < conf_flag->finger_stop_y[index] -  md->thr[CY_Y_MOVE_UP])) ? 1 : 0;
	TS_LOG_DEBUG("%s: flag_move_down = %d\n", __func__,flag_move_down);
	TS_LOG_DEBUG("%s: flag_move_up = %d\n", __func__,flag_move_up);

	//Last time is stop report
	if(conf_flag->stop_report_flag & (1 << index)){
		//moving up or moving down,then report.
		if(flag_move_up || flag_move_down){
			debug_status = 4;
			goto out_must_report;
		}
		//Not moving up or moving down & inside Line-C or Line-E,then stop report.
		if(flag_inside_c || flag_between_c_e){
			debug_status = 5;
			goto out_stop_report;
		}
		//Not moving up or moving down & overstep Line-E,then must report.
		if(flag_between_e_e){
			debug_status = 6;
			goto out_must_report;
		}
	...
	TS_LOG_ERR("%s:finger index: %d, some thing wrong,debug_status = %d.\n",__func__,index,debug_status);
	return 0; //keep report.

out_stop_report:
	conf_flag->stop_report_flag |=  (1 << index);
	conf_flag->must_report_flag &= ~(1 << index);
	TS_LOG_DEBUG("%s:finger index: %d, keep stopped,debug_status = %d.\n",__func__, index,debug_status);
	return -1;
out_must_report:
	conf_flag->stop_report_flag &= ~(1 << index);
	conf_flag->must_report_flag |=  (1 << index);
	TS_LOG_DEBUG("%s:finger index: %d, must report,debug_status = %d.\n",__func__,index,debug_status);
	return 1;
}
					...
					unsigned int *reprot_gesture_point_num)
{
	...
	if (!
	    (APP_ENABLE_GESTURE(GESTURE_CIRCLE_SLIDE) & cd->
	     easy_wakeup_gesture)) {
	...
	case CIRCLE_SLIDE_CLOCKWISE:
		if (IS_GESTURE_ENABLE
		    (GESTURE_CIRCLE_SLIDE, cd->easy_wakeup_gesture)) {
...
static int cyttsp5_gesture_report(struct cyttsp5_core_data *cd, int event,
				  int value)
{
	unsigned int reprot_gesture_key_value = 0;
	unsigned long lTimerCur = jiffies;
	static unsigned long lTimerPre;
	static unsigned long lTimerNext;
	...
	if (time_before(lTimerCur, lTimerPre + lTimerNext)) {
		TS_LOG_DEBUG
		    ("%s:Gesture Timer Interval is less than 1 senond!\n",
		     __func__);
		return 0;
	} else {
...
static int cyttp5_palm_report(struct cyttsp5_mt_data *md)
{
	...
	if (APP_ENABLE_GESTURE(GESTURE_PALM_COVERED) & cd->easy_wakeup_gesture) {
		...
		reprot_gesture_key_value =
		    cd->wakeup_keys->keys[GESTURE_PALM_COVERED];
		goto report_key;
	}
...
report_key:
	...
	TS_LOG_DEBUG("%s:reprot_gesture_key_value = %d", __func__,
		     reprot_gesture_key_value);
	input_report_key(cd->md.input, reprot_gesture_key_value, 1);
	input_sync(cd->md.input);
	input_report_key(cd->md.input, reprot_gesture_key_value, 0);
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (tch->abs[CY_TCH_E] == CY_EV_LIFTOFF) {
			TS_LOG_DEBUG("%s: t=%d e=%d lift-off\n",
	            ...
	            if (md->pdata->edge_algo_flag) {
				conf_flag->stop_report_flag &= ~(1 << t);
				conf_flag->must_report_flag &= ~(1 << t);
				conf_flag->finger_stop_y[t] = 0;
	            }
		...
		if (md->pdata->edge_algo_flag) {
			status = cyttsp5_mt_process_point(md, tch,t);
			if(status <0){
				TS_LOG_DEBUG("%s: stop to report the point,status = %d\n", __func__,status);
				continue;
			}
		}
...
cyttsp5_get_mt_touches_pr_tch:
		TS_LOG_DEBUG
		    ("%s: t=%d x=%d y=%d z=%d M=%d m=%d o=%d e=%d obj=%d tip=%d\n",
		     ...
		     tch->abs[CY_TCH_E], tch->abs[CY_TCH_O],
		     tch->abs[CY_TCH_TIP]);
	}

	if (md->mt_function.final_sync)
		md->mt_function.final_sync(md->input,
					   md->pdata->slots_max, mt_sync_count,
					   ids);

	md->num_prv_rec = num_cur_tch;
}

static void cyttsp5_mt_finger_up(struct cyttsp5_mt_data *md)
{
	int index = 0;
	struct cyttsp5_dege_conf_flag *conf_flag = &md->conf_flag;
	conf_flag->must_report_flag = 0;
	conf_flag->stop_report_flag = 0;
	for (index = 0; index < 10; index++)
	{
		conf_flag->finger_stop_y[index] = 0;
	}
}
...
static int cyttsp5_check_and_request_si(struct device *dev,
...
static int init_easy_wakeup_key_value(struct device *dev)
{
	...
	for (i = 0; i < cd->wakeup_keys->size; ++i) {
		TS_LOG_DEBUG("%s: wakeup_keys[%d] = %d", __func__, i,
			     cd->wakeup_keys->keys[i]);
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	TS_LOG_INFO
	    ("%s:set input dev p_max=%d slots_max=%d x_max=%d y_max=%d\n",
