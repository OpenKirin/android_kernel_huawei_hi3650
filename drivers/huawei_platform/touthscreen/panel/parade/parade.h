...
#define PARADE_TTHE_TUNER_GET_PANEL_DATA_FILE_NAME "get_panel_data"
#define TTHE_TUNER_MAX_BUF	(PARADE_MAX_PRBUF_SIZE * 3)
...
struct parade_core_data {
	...
	struct work_struct oem_work;
	struct work_struct get_rawdata_work;
	struct work_struct glove_switch_work;
	struct work_struct holster_switch_work;
	struct work_struct get_calibration_data_work;
	struct work_struct sdcard_udpate_work;

	struct work_struct power_ctrl_work;
	int    parade_power_ctrl_param;

	struct ts_oem_info_param *oem_info;
	struct ts_rawdata_info *rawdata_info;
	struct ts_glove_info *glove_info;
	struct ts_holster_info *holster_info;
	struct ts_calibration_data_info *calibration_data_info;
