...
static int ts_power_gpio_ref;
static u16 pre_status;
...
static int hideep_regulator_get(void)
{
	...
	if (1 == dat->vci_regulator_type) {
		g_istxxxx_data->tp_vci =
		    regulator_get(&platform_dev->dev, HIDEEP_VDD);
	...
	if (1 == dat->vddio_regulator_type) {
		g_istxxxx_data->tp_vddio =
		    regulator_get(&platform_dev->dev, HIDEEP_VBUS);
...
static int hideep_vci_enable(void)
{
	...
	if (!IS_ERR(g_istxxxx_data->tp_vci)) {
		...
		retval =
		    regulator_set_voltage(g_istxxxx_data->tp_vci, vol_vlaue,
					  vol_vlaue);
		if (retval < 0) {
			TS_LOG_ERR
...
static int hideep_vddio_enable(void)
{
	...
	retval =
	    regulator_set_voltage(g_istxxxx_data->tp_vddio, vol_vlaue,
				  vol_vlaue);
	if (retval < 0) {
...
static int hideep_pinctrl_get_init(void)
{
	...
	g_istxxxx_data->pins_default =
	    pinctrl_lookup_state(g_istxxxx_data->pctrl, "default");
	if (IS_ERR(g_istxxxx_data->pins_default)) {
		TS_LOG_ERR("failed to pinctrl lookup state default\n");
		ret = -EINVAL;
		goto err_pinctrl_put;
	}
	g_istxxxx_data->pins_idle =
	    pinctrl_lookup_state(g_istxxxx_data->pctrl, "idle");
...
static int hideep_pinctrl_select_normal(void)
{
	int retval = NO_ERR;
	retval =
	    pinctrl_select_state(g_istxxxx_data->pctrl,
				 g_istxxxx_data->pins_default);
	if (retval < 0) {
...
static int hideep_gpio_request(void)
{
	...
	if ((1 == dat->vci_gpio_type) && (1 == dat->vddio_gpio_type)) {
		if (dat->vci_gpio_ctrl == dat->vddio_gpio_ctrl) {
			retval =
			    gpio_request(dat->vci_gpio_ctrl, "ts_vci_gpio");
			if (retval) {
				TS_LOG_ERR
			...
			retval =
			    gpio_request(dat->vddio_gpio_ctrl, "ts_vddio_gpio");
			if (retval) {
				TS_LOG_ERR
...
static s32 hideep_get_event(struct ist510e *ts)
{
	...
	ret =
	    hideep_i2c_read(ts, TOUCH_COUNT_ADDR,
			    TOUCH_MAX_COUNT * sizeof(struct ist510e_touch_evt) +
			    2, (u8 *) ts->i2c_buf);
	...
	TS_LOG_DEBUG("touch = %d, key = %d, event = %d \n", ts->input_touch_nr,
		     ts->input_key_nr, ts->input_event_nr);
	if (ts->input_touch_nr > TOUCH_MAX_COUNT
	    || ts->input_key_nr > KEYS_MAX_COUNT) {
	...
	if (ts->input_touch_nr > 0) {
		info_size =
		    ts->input_touch_nr * sizeof(struct ist510e_touch_evt);
#ifdef HIDEEP_READ_I2C_CNT_DATA
		memcpy((u8 *) ts->input_evt, (u8 *) (ts->i2c_buf + 2),
		       ts->input_touch_nr * sizeof(struct ist510e_touch_evt));
#else
		ret =
		    hideep_i2c_read(ts, TOUCH_READ_START_ADDR, info_size,
				    (u8 *) ts->input_evt);
#endif
		if (ret < 0) {
			goto i2c_err;
		...
		if (ts->debug_evt) {
			ts->debug_index =
			    ts->debug_index >=
			    MAX_DEBUG_COUNT ? 0 : ts->debug_index;
			ts->debug_evt[ts->debug_index].count =
			    ts->input_touch_nr;
			ktime_get_ts(&ts->event_time[ts->debug_index]);
			memcpy(&ts->debug_evt[ts->debug_index].events[0],
			       (u8 *) ts->input_evt, info_size);
			ts->debug_index++;
			ts->debug_total++;
			ts->debug_total =
			    ts->debug_total >
			    2000000 ? 2000000 : ts->debug_total;
		}
#endif
	}
	if (ts->input_key_nr > 0) {
		uint32_t info_size =
		    ts->input_key_nr * sizeof(struct ist510e_touch_key);
		ret =
		    hideep_i2c_read(ts, KEY_READ_START_ADDR, info_size,
				    (u8 *) ts->input_key);
		if (ret < 0) {
			goto i2c_err;
		}
...
s32 hideep_get_image(struct ist510e *ts)
{
	...
	ret =
	    hideep_i2c_read(ts, VR_ADDR_IMAGE, debug_dev->im_size,
			    debug_dev->im_buff);
	if (ret < 0)
		goto i2c_err;
...
static int hideep_capability(struct input_dev *input_dev,
			     struct ts_device_data *ts_dev_data)
{
	u32 i = 0;
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     ts_dev_data->x_max_mt, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     ts_dev_data->y_max_mt, 0, 0);
	/* z value's range.*/
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, 65535, 0, 0);
	/* w value's range.*/
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, TOUCH_MAX_COUNT,
			     0, 0);
	return 0;
}
...
static int hideep_parse_config(struct device_node *device,
			       struct ts_device_data *data)
{
	...
	ret |=
	    of_property_read_u32(device, HIDEEP_TP_FUNC_FLAG,
				 &data->tp_func_flag);
	ret |=
	    of_property_read_u32(device, HIDEEP_VCI_GPIO_TYPE,
				 &data->vci_gpio_type);
	ret |=
	    of_property_read_u32(device, HIDEEP_VCI_REGULATOR_TYPE,
				 &data->vci_regulator_type);
	ret |=
	    of_property_read_u32(device, HIDEEP_VDDIO_GPIO_TYPE,
				 &data->vddio_gpio_type);
	ret |=
	    of_property_read_u32(device, HIDEEP_VDDIO_REGULATOR_TYPE,
				 &data->vddio_regulator_type);
	...
	ret =
	    of_property_read_u32(device, IST_RAWDATA_ARRANGE_SWAP,
				 &data->rawdata_arrange_swap);
	if (ret) {
	...
	ret =
	    of_property_read_u32(device, HIDEEP_UNIT_CAP_TEST_INTERFACE,
				 &data->unite_cap_test_interface);
	if (ret) {
	...
	ret =
	    of_property_read_u32(device, "support_3d_func",
				 &data->support_3d_func);
	if (ret) {
	...
	ret =
	    of_property_read_u32(device, "supported_func_indicater",
				 &data->supported_func_indicater);
	if (ret) {
	...
	if (1 == data->unite_cap_test_interface) {
		...
		for (index = 0; index < array_len; index++) {
			...
			if (ret) {
				TS_LOG_ERR
				    ("read index = %d,raw_data_limit = %s,ret = %d error,\n",
	...
	if (data->vci_gpio_type == 1) {
		data->vci_gpio_ctrl =
		    of_get_named_gpio(device, HIDEEP_VCI_GPIO_CTRL, 0);
	...
	if (data->vddio_gpio_type == 1) {
		data->vddio_gpio_ctrl =
		    of_get_named_gpio(device, HIDEEP_VDDIO_GPIO_CTRL, 0);
...
static int hideep_chip_detect(struct device_node *device,
			      struct ts_device_data *ts_data,
			      struct platform_device *ts_dev)
{
	...
	g_istxxxx_data->huawei_ts_data->easy_wakeup_info.sleep_mode =
	    TS_POWER_OFF_MODE;
	g_istxxxx_data->huawei_ts_data->easy_wakeup_info.easy_wakeup_gesture =
	    false;
	g_istxxxx_data->huawei_ts_data->easy_wakeup_info.easy_wakeup_flag =
	    false;
	g_istxxxx_data->huawei_ts_data->easy_wakeup_info.palm_cover_flag =
	    false;
	g_istxxxx_data->huawei_ts_data->easy_wakeup_info.palm_cover_control =
	    false;
	...
	g_istxxxx_data->apple_weight_workqueue =
	    create_singlethread_workqueue("apple_weight_wq");
	INIT_WORK(&g_istxxxx_data->apple_weight_work,
		  apple_weight_timeout_func);
	return NO_ERR;
err_aux:
	hideep_power_off();
err_pinctrl_get:
...
static int hideep_irq_bottom_half(struct ts_cmd_node *in_cmd,
				  struct ts_cmd_node *out_cmd)
{
	...
	TS_LOG_DEBUG("apple_weight_init = %d, input_event_nr = %d",
		     ts->apple_weight_init, ts->input_event_nr);
	if (ts->apple_weight_init && ts->input_event_nr) {
		ret =
		    hideep_i2c_read(ts,
				    TOUCH_READ_START_ADDR +
				    TOUCH_MAX_COUNT *
				    sizeof(struct ist510e_touch_evt) + 12, 4,
				    (u8 *) gesture);
	...
	} else if (TS_GESTURE_MODE == gesture_info->sleep_mode) {
		...
		if ((true == gesture_info->off_motion_on)
			...
			ret =
			    hideep_i2c_read(ts,
					    TOUCH_READ_START_ADDR +
					    TOUCH_MAX_COUNT *
					    sizeof(struct ist510e_touch_evt) +
					    12, 26, (u8 *) gesture);
			TS_LOG_DEBUG("gesture = 0x%02x\n", gesture[0]);
			switch (gesture[0]) {
			case HIDEEP_GES_DOUBLE_TAP:
				if (CHECK_ENABLE_GESTURE(GESTURE_DOUBLE_CLICK) &
				    gesture_info->easy_wakeup_gesture) {
			...
			case HIDEEP_GES_CHAR:
				...
				switch (gesture[1]) {
				case HIDEEP_CHAR_C:
					if (CHECK_ENABLE_GESTURE
					    (GESTURE_LETTER_c) & gesture_info->
					    easy_wakeup_gesture) {
						info->gesture_wakeup_value =
						    TS_LETTER_c;
					}
		...
		if ((true == gesture_info->off_motion_on)
			...
			for (i = 0; i < report_coordinate; i++) {
				x = *ps++;
				y = *ps++;
				gesture_info->easywake_position[i] =
				    (x << 16) | y;
				TS_LOG_DEBUG
				    ("x[%d] = %d, y[%d] = %d pos[%d] = 0x%x\n",
				     i, x, i, y, i,
				     gesture_info->easywake_position[i]);
			}
			...
			out_cmd->command = TS_INVAILD_CMD;
			command = 1;
			hideep_item_switch(TS_ACTION_WRITE, VR_LPWU, 1,
					   &command);
			command = 0;
	...
	finger = ts->input_evt;
	temp_status = 0;
	if ((ts->z_flag_calib2) && (info->cur_finger_number)
	    && (!ts->z_flag_ready)) {
	...
	TS_LOG_DEBUG
	    ("temp_status = 0x%x, pre_status = 0x%x, eor = 0x%x, result = 0x%x",
	     temp_status, pre_status, temp_status ^ pre_status,
	     (temp_status ^ pre_status) & temp_status);
	if ((temp_status != pre_status)
	    && ((temp_status ^ pre_status) & temp_status)) {
		TS_LOG_DEBUG("%s ROI readying\n", __FUNCTION__);
		ret =
		    hideep_i2c_read(g_istxxxx_data, VR_READ_ROI_DATA_ADDR, 49,
				    (u8 *) roi_buf);
		if (!ret) {
...
static int hideep_before_suspend(void)
{
	...
	if (ts->apple_weight_init) {
		...
		cmd = 0;
		if (hideep_item_switch
		    (TS_ACTION_WRITE, VR_SINGLE_TOUCH, 1, &cmd) < 0) {
			hideep_reset_ic();
		}
	}
	return NO_ERR;
...
static int hideep_fw_update_boot(char *file_name)
{
	...
	snprintf(firmware_name, sizeof(firmware_name), "ts/%s%s.bin", file_name,
		 "ist510e");
...
static int hideep_holster_switch(struct ts_holster_info *info)
{
	...
	TS_LOG_INFO
	    ("switch = %d, Holster window_info is (x0=%d,y0=%d)(x1=%d,y1=%d)\n",
...
static void apple_weight_timeout_func(struct work_struct *work)
{
	...
	do {
		if (!g_istxxxx_data->apple_weight_init) {
			TS_LOG_DEBUG
			    ("apple_weight_timeout_func thread finish\n");
			break;
		}
		mdelay(100);
		TS_LOG_DEBUG("apple_weight_timeout_func time out = %d",
...
static int hideep_single_touch_switch(struct ts_single_touch_info *info)
{
	...
	if (TS_ACTION_WRITE == info->op_action) {
		TS_LOG_DEBUG
		    ("hideep_single_touch_switch single_touch_switch = %d\n",
		...
		case 0:
			...
			hideep_item_switch(info->op_action, VR_SINGLE_TOUCH, 1,
					   &info->single_touch_switch);
			break;
		case 1:
			if (!g_istxxxx_data->apple_weight_init) {
				...
				if (g_istxxxx_data->apple_weight_workqueue)
					queue_work(g_istxxxx_data->
						   apple_weight_workqueue,
						   &g_istxxxx_data->
						   apple_weight_work);
			} else {
		...
		if (g_istxxxx_data->apple_weight_flag) {
			...
			hideep_i2c_read(g_istxxxx_data, IST_APPLE_WEIGHT_VALUE,
					2, (u8 *) &info->single_touch_switch);
		}
	}
	return ret;
}
...
static int hideep_glove_switch(struct ts_glove_info *info)
{
	...
	if (info->op_action == TS_ACTION_WRITE) {
		...
		ret =
		    hideep_item_switch(info->op_action, VR_GLOVE, 1,
				       &info->glove_switch);
...
static int hideep_roi_switch(struct ts_roi_info *info)
{
	...
	TS_LOG_INFO("%s: action = %d swtich = %d\n", __FUNCTION__,
		    info->op_action, info->roi_switch);
	if (!g_istxxxx_data) {
	...
	case TS_ACTION_WRITE:
		...
		ret =
		    hideep_i2c_write(g_istxxxx_data, VR_READ_ROI_EN_ADDR, 1,
				     &g_istxxxx_data->roi.enable);
		break;
	case TS_ACTION_READ:
...
static int hideep_wakeup_gesture_enable_switch(struct
...
static int hideep_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		memcpy(info->tp_test_type, dat->tp_test_type,
		       TS_CAP_TEST_TYPE_LEN);
...
struct ts_device_ops ts_hideep_ops = {
	...
	.chip_wakeup_gesture_enable_switch =
	    hideep_wakeup_gesture_enable_switch,
