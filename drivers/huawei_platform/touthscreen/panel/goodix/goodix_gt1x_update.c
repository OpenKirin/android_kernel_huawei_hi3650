...
u8 *gt1x_get_fw_data(u32 offset, int length)
{
	...
	if (update_info.update_type == UPDATE_TYPE_FILE) {
		...
		ret = vfs_read(update_info.fw_file, (char *)update_info.buffer,
						length, &update_info.fw_file->f_pos);
		if (ret < 0) {
			GTP_ERROR("Read data error:%d", ret);
			return NULL;
		}
...
_reset:        
    ...
    if (!retry) {
        GTP_INFO("Update abort due to i2c error.");
        return ERROR_CHECK;
    }
