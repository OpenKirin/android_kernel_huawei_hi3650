...
static int *rawdatabuf;
static int rawdata_size;
static int rawdata_index;
...
static int cypress_parse_config(struct device_node *device,
				struct ts_device_data *chip_data);
static int cypress_input_config(struct input_dev *input_dev);
static int cypress_irq_bottom_half(struct ts_cmd_node *in_cmd,
...
static int cypress_exec_cmd(u8 mode, u8 *cmd_buf, size_t cmd_size,
...
static int cypress_get_rawdata(struct ts_rawdata_info *info,
			       struct ts_cmd_node *out_cmd);
...
int cypress_algo_t1(struct ts_device_data *dev_data, struct ts_fingers *in_info,
					    ...
					    int size)
{
	...
	if (action)
		TS_LOG_DEBUG
		    ("[cypress debug] i2c action write addr:0x%x, size:%d\n",
		     addr, size);
	else
		TS_LOG_DEBUG
		    ("[cypress debug] i2c action read addr:0x%x, size:%d\n",
		     addr, size);
...
{
	int rc = 0;
	rc = core_data.cypress_chip_data->bops->bus_read(reg_addr, reg_len, buf,
							 len);
	if (rc < 0)
...
static int cypress_i2c_write(u8 addr, u8 *buf, u16 length)
{
	...
	rc = core_data.cypress_chip_data->bops->bus_write(wr_buf,
							  sizeof(wr_buf));
	if (rc < 0)
...
static int cypress_handshake(u8 mode)
{
	...
	if (mode & CY_HST_MODE_CHANGE) {
		TS_LOG_ERR
		    ("Host mode change bit set, NO handshake. mode = 0x%x\n",
		     mode);
		return NO_ERR;
	}
...
static int cypress_checkout_bl(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), mode,
				      sizeof(mode));
		if (rc < 0) {
			TS_LOG_ERR
			    ("Failed to read register 0x00, i = %d, rc = %d\n",
			     i, rc);
		} else {
			...
			if (IS_BOOTLOADER(mode[0])) {
				...
				TS_LOG_ERR
				    ("NOT in bootloader mode!mode[0] = %d, i = %d\n",
				     mode[0], i);
				cypress_soft_reset();
				rc = -EIO;
			}
...
static int cypress_get_glove_switch(u8 *glove_switch)
{
	...
	TS_LOG_INFO("return_buf=0x%x,0x%x,0x%x\n", read_return_buf[0],
		    read_return_buf[1], read_return_buf[2]);
...
static int cypress_set_glove_switch(u8 glove_switch)
{
	...
	rc = cypress_exec_cmd(CY_HST_OPERATE, write_cmd_buf,
			      sizeof(write_cmd_buf), write_return_buf,
			      sizeof(write_return_buf));
	if (rc < 0) {
...
static int cypress_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		if (rc < 0) {
			TS_LOG_ERR("set glove switch(%d), failed : %d",
				   info->glove_switch, rc);
		}
		break;
	case TS_ACTION_READ:
	...
	case TS_ACTION_WRITE:
		...
		if (rc < 0) {
			TS_LOG_ERR("set holster switch(%d), failed : %d",
				   info->holster_switch, rc);
		}
		break;
	default:
		TS_LOG_ERR("invalid  holster switch status: %d",
			   info->holster_switch);
		rc = -EINVAL;
		break;
	}
...
static int cypress_hand_detect(struct ts_hand_info *info)
{
	...
	if (rc < 0) {
		TS_LOG_ERR
		    ("send execute handness_detection init command fail!\n");
		goto err;
	}
...
static int cypress_resume(void)
{
	...
	if (rc < 0) {
		...
		udelay(2000);
		rc = gpio_direction_input(core_data.cypress_chip_data->
					  irq_gpio);
		if (rc < 0) {
	...
	for (i = 0; i < I2C_RW_TRIES - 1; i++) {
		...
		if (rc < 0) {
			TS_LOG_ERR
			    ("goto CY_HST_OPERATE failed[%d], check again\n",
			     i);
		} else {
			break;
		}
...
static int cypress_sysinfo_offsets(void)
{
	...
	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr),
			      (u8 *) &core_data.sysinfo.sysinfo_data,
			      sizeof(core_data.sysinfo.sysinfo_data));
	if (rc < 0) {
		TS_LOG_ERR("failed to read HST_MODE register, rc = %d\n", rc);
		return rc;
	}
	/* convert sysinfo data offset bytes into integers */
	core_data.sysinfo.sysinfo_merged_data.map_sz =
	    merge_bytes(core_data.sysinfo.sysinfo_data.map_szh,
			core_data.sysinfo.sysinfo_data.map_szl);
	core_data.sysinfo.sysinfo_merged_data.cydata_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.cydata_ofsh,
			core_data.sysinfo.sysinfo_data.cydata_ofsl);
	core_data.sysinfo.sysinfo_merged_data.test_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.test_ofsh,
			core_data.sysinfo.sysinfo_data.test_ofsl);
	core_data.sysinfo.sysinfo_merged_data.pcfg_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.pcfg_ofsh,
			core_data.sysinfo.sysinfo_data.pcfg_ofsl);
	core_data.sysinfo.sysinfo_merged_data.opcfg_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.opcfg_ofsh,
			core_data.sysinfo.sysinfo_data.opcfg_ofsl);
	core_data.sysinfo.sysinfo_merged_data.ddata_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.ddata_ofsh,
			core_data.sysinfo.sysinfo_data.ddata_ofsl);
	core_data.sysinfo.sysinfo_merged_data.mdata_ofs =
	    merge_bytes(core_data.sysinfo.sysinfo_data.mdata_ofsh,
			core_data.sysinfo.sysinfo_data.mdata_ofsl);

	TS_LOG_DEBUG
	    ("cydata_ofs:0x%4zx, test_ofs:0x%4zx, pcfg_ofs:0x%zx, opcfg_ofs:0x%zx, ddata_ofs:0x%zx, mdata_ofs:0x%zx\n",
	     core_data.sysinfo.sysinfo_merged_data.cydata_ofs,
	     core_data.sysinfo.sysinfo_merged_data.test_ofs,
	     core_data.sysinfo.sysinfo_merged_data.pcfg_ofs,
	     core_data.sysinfo.sysinfo_merged_data.opcfg_ofs,
...
static int cypress_sysinfo_test(void)
{
	...
	TS_LOG_DEBUG("pcfg_ofs = 0x%zx, test_ofs = 0x%zx\n",
		     core_data.sysinfo.sysinfo_merged_data.pcfg_ofs,
		     core_data.sysinfo.sysinfo_merged_data.test_ofs);
	test_size =
	    core_data.sysinfo.sysinfo_merged_data.pcfg_ofs -
	...
	if (test == NULL) {
		TS_LOG_ERR("Failed to alloc test memory, test_size = %zu\n",
			   test_size);
		rc = -ENOMEM;
		goto out;
	}

	rc = cypress_i2c_read((u8 *) &core_data.sysinfo.sysinfo_merged_data.
			      test_ofs, 1, (u8 *) test, test_size);
	if (rc < 0) {
...
static int cypress_sysinfo_cydata(void)
{
	...
	TS_LOG_DEBUG("test_ofs = 0x%zx, cydata_ofs = 0x%zx\n",
		     core_data.sysinfo.sysinfo_merged_data.test_ofs,
		     core_data.sysinfo.sysinfo_merged_data.cydata_ofs);
	cydata_size =
	    core_data.sysinfo.sysinfo_merged_data.test_ofs -
	...
	if (buf == NULL) {
		TS_LOG_ERR
		    ("Failed to alloc cydata buffer(null), cydata_size = %zu\n",
		     cydata_size);
		rc = -ENOMEM;
		goto out;
	}
	...
	TS_LOG_DEBUG("ttpidh:0x%x, ttpidl:0x%x\n",
		     core_data.sysinfo.cydata.ttpidh,
		     core_data.sysinfo.cydata.ttpidl);
	TS_LOG_DEBUG("fw_ver_major:0x%x, fw_ver_minor:0x%x\n",
		     core_data.sysinfo.cydata.fw_ver_major,
		     core_data.sysinfo.cydata.fw_ver_minor);
	for (i = 0; i < CY_NUM_REVCTRL; i++) {
		TS_LOG_DEBUG("revctrl[%d]:0x%x\n", i,
			     core_data.sysinfo.cydata.revctrl[i]);
	}
	TS_LOG_DEBUG("blver_major:0x%x, blver_minor:0x%x\n",
		     core_data.sysinfo.cydata.blver_major,
		     core_data.sysinfo.cydata.blver_minor);
	TS_LOG_DEBUG("jtag_si_id[3-0]:0x%x, 0x%x, 0x%x, 0x%x\n",
		     core_data.sysinfo.cydata.jtag_si_id3,
		     core_data.sysinfo.cydata.jtag_si_id2,
		     core_data.sysinfo.cydata.jtag_si_id1,
		     core_data.sysinfo.cydata.jtag_si_id0);
	TS_LOG_DEBUG("mfgid_sz:0x%x\n", core_data.sysinfo.cydata.mfgid_sz);
	TS_LOG_DEBUG("cyito_idh:0x%x, cydata.cyito_idl:0x%x\n",
		     core_data.sysinfo.cydata.cyito_idh,
		     core_data.sysinfo.cydata.cyito_idl);
	TS_LOG_DEBUG("cyito_verh:0x%x, cydata.cyito_verl:0x%x\n",
		     core_data.sysinfo.cydata.cyito_verh,
		     core_data.sysinfo.cydata.cyito_verl);
	TS_LOG_DEBUG("ttsp_ver_major:0x%x, ttsp_ver_minor:0x%x\n",
		     core_data.sysinfo.cydata.ttsp_ver_major,
		     core_data.sysinfo.cydata.ttsp_ver_minor);
...
static int cypress_sysinfo_pcfg(void)
{
	...
	TS_LOG_DEBUG("opcfg_ofs = 0x%zx, pcfg_ofs = 0x%zx\n",
		     core_data.sysinfo.sysinfo_merged_data.opcfg_ofs,
		     core_data.sysinfo.sysinfo_merged_data.pcfg_ofs);
	pcfg_size =
	    core_data.sysinfo.sysinfo_merged_data.opcfg_ofs -
	...
	TS_LOG_DEBUG("electrodes_x:%d,electrodes_y:%d\n", pcfg->electrodes_x,
		     pcfg->electrodes_y);
	TS_LOG_DEBUG("pcfg_size:%zu\n", pcfg_size);
	TS_LOG_DEBUG("max_x:0x%4zX\n",
		     core_data.sysinfo.sysinfo_merged_data.max_x);
	TS_LOG_DEBUG("x_origin:%zu (%s)\n",
		     core_data.sysinfo.sysinfo_merged_data.x_origin,
		     core_data.sysinfo.sysinfo_merged_data.x_origin ==
		     CY_NORMAL_ORIGIN ? "left corner" : "right corner");
	TS_LOG_DEBUG("max_y:0x%4zX\n",
		     core_data.sysinfo.sysinfo_merged_data.max_y);
	TS_LOG_DEBUG("y_origin:%zu (%s)\n",
		     core_data.sysinfo.sysinfo_merged_data.y_origin,
		     core_data.sysinfo.sysinfo_merged_data.y_origin ==
		     CY_NORMAL_ORIGIN ? "upper corner" : "lower corner");
	TS_LOG_DEBUG("max_p:0x%4zX\n",
		     core_data.sysinfo.sysinfo_merged_data.max_p);
...
static int cypress_sysinfo_opcfg(void)
{
	...
	TS_LOG_DEBUG("ddata_ofs = 0x%zx, opcfg_ofs = 0x%zx\n",
		     core_data.sysinfo.sysinfo_merged_data.ddata_ofs,
		     core_data.sysinfo.sysinfo_merged_data.opcfg_ofs);
	opcfg_size =
	    core_data.sysinfo.sysinfo_merged_data.ddata_ofs -
	...
	if (opcfg == NULL) {
		TS_LOG_ERR("Failed to alloc opcfg memory, opcfg_size = %zu\n",
			   opcfg_size);
		rc = -ENOMEM;
		goto out;
	}
	...
	rc = cypress_i2c_read((u8 *) &core_data.sysinfo.sysinfo_merged_data.
			      opcfg_ofs, 1, (u8 *) opcfg, opcfg_size);
	if (rc < 0) {
		TS_LOG_ERR("Failed to read cydata rc = %d\n", rc);
		goto free_opcfg;
	}
	...
	for (abs = CY_TCH_X; abs < CY_NUM_TCH_FIELDS; abs++) {
		core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].ofs =
		    opcfg->tch_rec_old[abs].loc & CY_BYTE_OFS_MASK;
		core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].size =
		    cypress_bits_to_bytes((int)opcfg->tch_rec_old[abs].size,
					  (int *)&core_data.sysinfo.
					  sysinfo_merged_data.tch_abs[abs].max);
		core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].bofs =
		    (opcfg->tch_rec_old[abs].
		     loc & CY_BOFS_MASK) >> CY_BOFS_SHIFT;
	}
	core_data.sysinfo.sysinfo_merged_data.mode_size =
	    core_data.sysinfo.sysinfo_merged_data.tt_stat_ofs + 1;
	...
	for (abs = 0; abs < CY_TCH_NUM_ABS; abs++) {
		...
		TS_LOG_DEBUG("ofs =%2zu\n",
			     core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].
			     ofs);
		TS_LOG_DEBUG("siz =%2zu\n",
			     core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].
			     size);
		TS_LOG_DEBUG("max =%2zu\n",
			     core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].
			     max);
		TS_LOG_DEBUG("bofs=%2zu\n",
			     core_data.sysinfo.sysinfo_merged_data.tch_abs[abs].
			     bofs);
	}

	TS_LOG_DEBUG("cmd_ofs =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.cmd_ofs);
	TS_LOG_DEBUG("rep_ofs =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.rep_ofs);
	TS_LOG_DEBUG("rep_sz =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.rep_sz);
	TS_LOG_DEBUG("tt_stat_ofs =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.tt_stat_ofs);
	TS_LOG_DEBUG("tch_rec_size =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.tch_rec_size);
	TS_LOG_DEBUG("max_tchs =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.max_tchs);
	TS_LOG_DEBUG("mode_size =%4zu\n",
		     core_data.sysinfo.sysinfo_merged_data.mode_size);
	TS_LOG_DEBUG("data_size =%4zu\n",
...
{
	int rc = 0;
	TS_LOG_DEBUG("xy_mode_size = %zu, xy_data_size = %zu\n",
		     core_data.sysinfo.sysinfo_merged_data.mode_size,
		     core_data.sysinfo.sysinfo_merged_data.data_size);
	core_data.sysinfo.xy_mode =
	    kzalloc(core_data.sysinfo.sysinfo_merged_data.mode_size,
		    GFP_KERNEL);
	if (core_data.sysinfo.xy_mode == NULL) {
		TS_LOG_ERR("Failed to alloc memory for xy_mode, size is %zu\n",
			   core_data.sysinfo.sysinfo_merged_data.mode_size);
		return -1;
	}
	core_data.sysinfo.xy_data =
	    kzalloc(core_data.sysinfo.sysinfo_merged_data.data_size,
		    GFP_KERNEL);
	if (core_data.sysinfo.xy_data == NULL) {
		TS_LOG_ERR("Failed to alloc memory for xy_data, size is %zu\n",
			   core_data.sysinfo.sysinfo_merged_data.data_size);
		rc = -1;
		goto err;
	}
...
static int cypress_switch_to_operate(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			TS_LOG_ERR
			    ("failed to set mode to Operational rc = %d\n", rc);
			continue;
		}
		msleep(2);
		rc = check_hst_mode(CY_HST_OPERATE);
		if (rc < 0) {
			TS_LOG_ERR
			    ("check operate failed, try again, rc = %d, i = %d\n",
			     rc, i);
		} else {
...
static int cypress_read_config_block(u8 ebid, u16 row, u8 *data, u16 length)
{
	...
	rc = cypress_exec_cmd(CY_HST_CAT, command_buf, 6, return_buf,
			      length + 7);
	if (rc) {
...
static int cypress_get_ttconfig_version(u16 *version)
{
	...
	rc = cypress_read_config_block(CY_TCH_PARM_EBID, 0, data,
...
static int cypress_switch_to_cat(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			TS_LOG_ERR
			    ("wrong in cat mode, try again, rc = %d, i = %d\n",
			     rc, i);
		} else {
...
static int cypress_get_ttconfig_info(void)
{
	...
	TS_LOG_DEBUG
	    ("TT Config Version:%04X Length:%d Max Length:%d CRC:%04X\n",
	     core_data.sysinfo.ttconfig.version,
	     core_data.sysinfo.ttconfig.length,
			    ...
			    u8 *return_buf, size_t return_buf_size)
{
	...
	for (i = 0; i < I2C_RW_TRIES + 2; i++) {
		...
		rc = cypress_i2c_read((u8 *) &cmd_ofs, 1, &command,
				      sizeof(command));
		if (rc < 0) {
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			...
			if (!core_data.complete_cmd) {
				TS_LOG_ERR
				    ("command is not completed, complete_cmd = %d, i =%d\n",
	...
	reg_addr = cmd_ofs + 1;
	rc = cypress_i2c_read((u8 *) &reg_addr, 1, return_buf,
			      return_buf_size);
	if (rc < 0) {
...
static int set_hst_mode(u8 new_mode)
{
	...
	if (rc < 0)
		TS_LOG_ERR
		    ("Failed to write mode change, rc = %d, mode(value) = 0x%x\n",
		     rc, mode);
...
static int cypress_exit_bl_into_sysinfo(void)
{
	...
	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), output_packet,
			      sizeof(output_packet));
	if (rc < 0) {
	...
	if (0 == memcmp(output_packet, ldr_err_app, sizeof(ldr_err_app))) {
		TS_LOG_ERR
		    ("output packet equal to ldr_err_app, force update fw\n");
	...
	for (i = 0; i < I2C_RW_TRIES * 10; i++) {
		...
		if (rc < 0) {
			TS_LOG_DEBUG
			    ("not in sysinfo mode, rc = %d, i = %d. wait 200ms read again\n",
			     rc, i);
			msleep(200);
		} else {
...
static int cypress_get_status(u8 *buf, int size, unsigned long timeout_ms)
{
	...
	if (timeout_ms != 0) {
		...
		for (tries = 0; tries < I2C_RW_TRIES; tries++) {
			rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), buf,
					      size);
			/* retry if bus read error or status byte shows not ready */
			if ((rc < 0) || buf[1] == CY_COMM_BUSY
			    || buf[1] == CY_CMD_BUSY
...
static int cypress_ldr_parse_row(u8 *row_buf,
 ...
 */
static int cypress_check_fw_version(u32 fw_ver_new, u32 fw_revctrl_new_h,
				    u32 fw_revctrl_new_l)
{
	...
	fw_revctrl_img_h =
	    be32_to_cpu(*(u32 *) (core_data.sysinfo.cydata.revctrl + 0));
	...
	fw_revctrl_img_l =
	    be32_to_cpu(*(u32 *) (core_data.sysinfo.cydata.revctrl + 4));
...
static bool cypress_check_version(const struct firmware *fw)
{
	...
	upgrade_img =
	    cypress_check_fw_version(fw_ver_new, fw_revctrl_new_h,
				     fw_revctrl_new_l);
...
static int cypress_initialize_device(void)
{
	...
	for (i = 0; i < try_cnt; i++) {
		...
		if (!rc) {
			TS_LOG_INFO
			    ("cypress device goto SI mode successfully\n");
			break;
		}
	}
	if (i == try_cnt) {
...
static int cypress_dl_source_init(int fw_size)
{
	...
	core_data.ld.row_buf_size =
	    1024 > CY_MAX_PRBUF_SIZE ? 1024 : CY_MAX_PRBUF_SIZE;
	...
	core_data.ld.row_image =
	    kzalloc(sizeof(struct cypress_hex_image), GFP_KERNEL);
	...
	core_data.ld.dev_id =
	    kzalloc(sizeof(struct cypress_dev_id), GFP_KERNEL);
...
static int cypress_ldr_enter_result(void)
{
	...
	if (status == ERROR_SUCCESS) {
		core_data.ld.dev_id->bl_ver =
		    status_buf[11] << 16 | status_buf[10] << 8 | status_buf[9]
		    << 0;
		core_data.ld.dev_id->rev_id = status_buf[8] << 0;
		core_data.ld.dev_id->silicon_id =
		    status_buf[7] << 24 | status_buf[6] << 16 | status_buf[5] <<
		    8 | status_buf[4] << 0;
		rc = 0;
	} else {
	...
	TS_LOG_DEBUG("status=%d, bl_ver=%08X rev_id=%02X silicon_id=%08X\n",
		     status, core_data.ld.dev_id->bl_ver,
		     core_data.ld.dev_id->rev_id,
		     core_data.ld.dev_id->silicon_id);
...
static int cypress_ldr_init_command(void)
{
	...
	memcpy(&ldr_init_cmd[i], cypress_security_key,
	       sizeof(cypress_security_key));
...
static int cypress_prog_row_command(u8 *image_buf)
{
	...
	TS_LOG_DEBUG("p=%p buf=%p buf[0]=%02X\n", image_buf,
		     core_data.ld.row_buf, core_data.ld.row_buf[0]);
				  ...
				  size_t status_size, unsigned long timeout_ms)
{
	...
	if (timeout_ms > 0) {
		...
		if (rc < 0 || status_buf[0] != CY_START_OF_PACKET) {
			TS_LOG_ERR
			    ("Error getting status, rc = %d, status_buf[0]=%02X\n",
			     rc, status_buf[0]);
			if (!(rc < 0))
				rc = -EIO;
			return rc;
...
static int cypress_ldr_init_result(u8 *stat_ret, size_t num_stat_byte,
				   size_t status_size, unsigned long timeout_ms)
{
	return cypress_command_result(stat_ret, num_stat_byte, status_size,
				      timeout_ms);
}
...
LOOP:
	...
	case FW_UP_BL_EXIT_RESULT:
		rc = cypress_ldr_exit_result(NULL, 0, CY_CMD_LDR_EXIT_STAT_SIZE,
					     0);
		if (rc < 0) {
			state = FW_UP_FAIL;
		} else {
...
static int cypress_firmware_calibrate(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR
		    ("Unable to execute Mutual Capacitance Button calibrate command.\n");
		goto out;
	}
	if (return_buf[0] != 0) {
		TS_LOG_ERR
		    ("Mutual Capacitance Button calibrate command unsuccessful\n");
		rc = -EINVAL;
		goto out;
	}
	...
	if (rc < 0) {
		TS_LOG_ERR
		    ("Unable to execute Self Capacitance calibrate command.\n");
		goto out;
	}
	if (return_buf[0] != 0) {
...
static int cypress_fw_update_boot(char *file_name)
{
	...
	rc = cypress_load_app(&(fw_entry->data[header_size + 1]),
			      fw_entry->size - (header_size + 1));
	if (rc < 0)
...
static int cypress_gpio_request(void)
{
	...
	rc = gpio_request(core_data.cypress_chip_data->reset_gpio,
			  "ts_reset_gpio");
	if (rc < 0) {
		TS_LOG_ERR("Fail to request gpio:%d. rc = %d\n",
			   core_data.cypress_chip_data->reset_gpio, rc);
		goto out;
	}
...
static void cypress_get_touch_axis(int *axis, int size, int max, u8 *xy_data,
				   int bofs)
{
	int nbyte;
	int next;
...
static int cypress_xy_worker(struct ts_fingers *info)
{
	...
	rep_stat =
	    core_data.sysinfo.xy_mode[core_data.sysinfo.sysinfo_merged_data.
				      rep_ofs + 1];
	tt_stat =
	    core_data.sysinfo.xy_mode[core_data.sysinfo.sysinfo_merged_data.
				      tt_stat_ofs];
	TS_LOG_DEBUG("hst_mode(xy_mode(%d)) = 0x%x, rep_len(xy_mode(%zu)) = 0x%x"
		     "rep_stat (xy_mode(%zu)) = 0x%x, tt_stat(xy_mode(%zu)) = 0x%x",
		     CY_REG_BASE, hst_mode,
		     core_data.sysinfo.sysinfo_merged_data.rep_ofs, rep_len,
		     core_data.sysinfo.sysinfo_merged_data.rep_ofs + 1,
		     rep_stat,
		     core_data.sysinfo.sysinfo_merged_data.tt_stat_ofs,
		     tt_stat);
	...
	TS_LOG_DEBUG
	    ("num_cur_tch = %d, sysinfo_reg_addr = 0x%x, tch_rec_size(each touch record size) = 0x%zx\n",
	     num_cur_tch, sysinfo_reg_addr,
	     core_data.sysinfo.sysinfo_merged_data.tch_rec_size);
	...
	/* read touches */
	if (num_cur_tch > 0) {
		rc = cypress_i2c_read(&sysinfo_reg_addr, 1,
				      core_data.sysinfo.xy_data,
				      num_cur_tch *
				      core_data.sysinfo.sysinfo_merged_data.
				      tch_rec_size);
		if (rc < 0) {
			TS_LOG_ERR
			    ("Failed to read on touch regs, sysinfo_reg_addr = 0x%x, size = %d, rc = %zu\n",
			     rc, sysinfo_reg_addr,
			     num_cur_tch *
			     core_data.sysinfo.sysinfo_merged_data.
			     tch_rec_size);
	...
	if (IS_BAD_PKT(rep_stat)) {
		TS_LOG_INFO
		    ("Invalid buffer detected, report data invalid, rep_stat = 0x%x\n",
		     rep_stat);
		return rc;
	}
	if (IS_LARGE_AREA(tt_stat)) {
		TS_LOG_INFO
		    ("Large area detected, set current num to 0, tt_stat = 0x%x\n",
		     tt_stat);
		num_cur_tch = 0;
	}
	if (num_cur_tch > core_data.sysinfo.sysinfo_merged_data.max_tchs) {
		if (num_cur_tch > CY_MAX_TCH) {
			...
			TS_LOG_ERR
			    ("(n=%d c=%zu), too many tch, set num_cur_tch to max tch",
			     num_cur_tch,
			     core_data.sysinfo.sysinfo_merged_data.max_tchs);
			num_cur_tch =
			    core_data.sysinfo.sysinfo_merged_data.max_tchs;
	...
	for (n_finger = 0; n_finger < num_cur_tch; n_finger++) {
		for (abs = CY_TCH_X; abs < CY_TCH_NUM_ABS; abs++) {
			cypress_get_touch_axis(&tch.abs[abs],
					       core_data.sysinfo.
					       sysinfo_merged_data.tch_abs[abs].
					       size,
					       core_data.sysinfo.
					       sysinfo_merged_data.tch_abs[abs].
					       max,
		...
		TS_LOG_DEBUG("X = %d, Y = %d, P = %d, T = %d, E = %d\n",
			     ...
			     tch.abs[CY_TCH_P], tch.abs[CY_TCH_T],
			     tch.abs[CY_TCH_E]);
		...
		TS_LOG_DEBUG("outdata: x = %d, y = %d, pressure = %d\n",
			     ...
			     info->fingers[touch_id].y,
			     info->fingers[touch_id].pressure);
	}
...
static int cypress_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, CY_ABS_MIN_X,
			     CY_ABS_MAX_X, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, CY_ABS_MIN_Y,
			     CY_ABS_MAX_Y, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, CY_ABS_MIN_P,
			     CY_ABS_MAX_P, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, CY_ABS_MIN_T,
			     CY_ABS_MAX_T, 0, 0);
				   ...
				   struct ts_cmd_node *out_cmd)
{
	...
	if (unlikely(mode[0] == CY_CORE_BL_HOST_SYNC_BYTE)) {
		TS_LOG_DEBUG
		    ("False interrupt in bootloader mode, mode[0] = 0x%x\n",
		     mode[0]);
		return NO_ERR;
	}
 ...
 */
static int cypress_retrieve_scan_data(int readOffset, int numElement,
				      u8 dataType, u8 *return_buf)
{
	...
	cmd_buf[5] = dataType;
	rc = cypress_exec_cmd(CY_HST_CAT, cmd_buf,
			      CY_CMD_CAT_RET_PANEL_DATA_CMD_SZ, return_buf,
...
static int cypress_check_range(enum check_data_type type, int endian,
	...
	for (index = 8; index < size; ++index) {
		if (endian == B_ENDIAN) {
			if (0 == index % datasize) {	/*high byte*/
				...
				temp |=
				    core_data.rawdate.ic_buf[index] << 8 *
				    (datasize - 1);
				if (datasize == 1) {
...
static int cypress_get_data_and_check(enum check_data_type type, int offset)
{
	...
	rc = cypress_retrieve_scan_data(offset,
					...
					core_data.rawdate.heatmap.dataType,
					return_buf);
	if (rc < 0) {
	...
	if (readByte >= I2C_BUF_MAX_SIZE) {
		rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr),
				      core_data.rawdate.ic_buf,
				      I2C_BUF_MAX_SIZE);
		dataIdx = I2C_BUF_MAX_SIZE;
	} else {
		rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr),
				      core_data.rawdate.ic_buf, readByte);
		dataIdx = readByte;
	}
	if (rc < 0) {
		TS_LOG_ERR("Error on read rc = %d\n", readByte);
	...
	while (leftOverElement > 0) {
		/* get the data */
		rc = cypress_retrieve_scan_data(readElementOffset,
						leftOverElement,
						core_data.rawdate.heatmap.
						dataType, return_buf);
		if (rc < 0)
			return 0;
		...
		if (readByte >= (I2C_BUF_MAX_SIZE - elementStartOffset)) {
			rc = cypress_i2c_read(&elementStartOffset, 1,
					      (u8 *) (core_data.rawdate.ic_buf +
						      dataIdx),
					      I2C_BUF_MAX_SIZE -
					      elementStartOffset);
	...
	core_data.rawdate.ic_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_H +
				 cmdParam_ofs] = HI_BYTE(readElementOffset);
	core_data.rawdate.ic_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_L +
				 cmdParam_ofs] = LO_BYTE(readElementOffset);
...
static int cypress_check_raw_data(void)
{
	...
	for (i = 0; i < 2; ++i) {
		...
		if (rc < 0)
			return 0;
		/* retrieve scan data */
		rc = cypress_get_data_and_check(type,
			       ...
			       struct ts_cmd_node *out_cmd)
{
	...
	rawdata_size =
	    ...
	    core_data.sysinfo.pcfg.electrodes_x +
	    core_data.sysinfo.pcfg.electrodes_y;
...
static int cypress_check_status(void)
{
	...
	u8 rep_stat_addr =
	    CY_REG_BASE + core_data.sysinfo.sysinfo_merged_data.rep_ofs + 1;
	u8 mode = 0;
	u8 rep_stat = 0;

	if (core_data.process_state == CY_STATE_NORMAL) {
		rc = cypress_i2c_read(&hst_mode_addr, sizeof(hst_mode_addr),
				      &mode, 1);
		rc1 =
		    cypress_i2c_read(&rep_stat_addr, sizeof(rep_stat_addr),
				     &rep_stat, 1);
		if ((rc < 0) || (rc1 < 0)) {
			TS_LOG_ERR
			    ("failed to access device in watchdog timer rc=%d, rc1=%d\n",
			     rc, rc1);
			rc = (rc < 0) ? rc : rc1;
		} else {
			TS_LOG_DEBUG
			    ("mode=0x%02x, rep_stat=0x%02x, rep_ofs=0x%02zx\n",
			     mode, rep_stat,
			     core_data.sysinfo.sysinfo_merged_data.rep_ofs);
			if (IS_BOOTLOADER(mode) && IS_BOOTLOADER(rep_stat)) {
				TS_LOG_ERR
				    ("device found in bootloader mode when operational mode, rep_stat=0x%02X\n",
				     rep_stat);
				rc = -EINVAL;
			}
		}
