...
#define N_HARD_RESET_COUNT  0//2  // BQ - change to zero and violate PD spec to fix issue with Huawei PD charger.
...
#define T_TYPEC_SINK_WAIT_CAP_MS     600   /* 310 - 620 ms */
	...
	switch (dev->rx_msg_type)
	{
		...
		case CTRL_MSG_TYPE_ACCEPT:
			...
			else if (*dev->current_state == PE_VCS_SEND_SWAP)
			{
				...
				if (tcpm_is_vconn_source(dev->port))
				{
...
static void pe_src_startup_entry(usb_pd_port_t *dev)
{
	...
	printk("TUSB422 dual role change - SRC\n");
#endif
...
static void pe_src_transition_to_default_exit(usb_pd_port_t *dev)
{
	if (dev->vconn_enable)
	{
		// Enable VCONN if it was previously enabled before Hard Reset.
...
static void pe_vcs_evaluate_swap_entry(usb_pd_port_t *dev)
{
	...
	else /* Not currently VCONN source */
	{
...
#ifdef CONFIG_DUAL_ROLE_USB_INTF
	dual_role_instance_changed(tusb422_dual_role_phy);
	printk("TUSB422 dual role change - SNK\n");
#endif
...
//	tcpm_set_autodischarge_disconnect(dev->port, true);  // BQ - remove for charger.  Put back for Ellisys.
...
static void pe_snk_ready_entry(usb_pd_port_t *dev)
{
	...
	if (tcpm_is_vconn_source(dev->port))
	{
		// Enable PD receive including SOP'/SOP".
		tcpm_enable_pd_receive(dev->port, true, true);
...
static void pe_snk_hard_reset_entry(usb_pd_port_t *dev)
{
	...
	// Disable sink VBUS per TCPC spec.
//	tcpm_snk_vbus_disable(dev->port);    // BQ - Remove to fix phone reboot loop with powered-off phone and PD charger.
...
static void pe_unattached_entry(usb_pd_port_t *dev)
{
	...
	{
		printk("TUSB422 dual role change - Unattach\n");

		dual_role_instance_changed(tusb422_dual_role_phy);
	}
#endif

	tusb422_stop_polling();
