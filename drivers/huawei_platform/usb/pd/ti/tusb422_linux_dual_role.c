									  ...
									  unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_VCONN_SUPPLY:
		if (tcpc_dev->flags & TC_FLAGS_TEMP_ROLE)
			*val = prop_vconn;
		else if (tcpm_is_vconn_source(tcpc_dev->port))
									  ...
									  const unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_VCONN_SUPPLY:
		if (((*val == DUAL_ROLE_PROP_VCONN_SUPPLY_NO) && tcpm_is_vconn_source(pd_dev->port))||
			((*val == DUAL_ROLE_PROP_VCONN_SUPPLY_YES) && !tcpm_is_vconn_source(pd_dev->port))) {
