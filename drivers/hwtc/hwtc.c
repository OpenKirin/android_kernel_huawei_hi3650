...
static bool hwtc_is_enable;
static struct kobject *hwtc_kobj;
...
static uint32_t hwtc_variance(struct hwtc_time_t time[], uint32_t count,
			      uint32_t average)
{
	uint32_t variance = 0;
	int i;
	...
	for (i = 0; i < count; i++) {
		variance +=
		    (time[i].time_delta - average) * (time[i].time_delta -
						      average);
	}
...
Return:         -1 : error, 0 : ok
********************************************************************************/
static int hwtc_get_statistic(struct hwtc_entry_t *entry,
...
static int hwtc_open(struct inode *inode, struct file *filp)
{
	...
	pd = (struct hwtc_private_data_t *)
	    kmalloc(sizeof(struct hwtc_private_data_t), GFP_KERNEL);
	if (NULL == pd) {
	...
	pd->id_tbl =
	    (uint32_t *) kmalloc(sizeof(uint32_t) * ID_TABLE_SIZE, GFP_KERNEL);
	if (NULL == pd->id_tbl) {
...
static int32_t hwtc_record_id(int32_t id, struct hwtc_private_data_t *pd)
{
	...
	if (pd->index < pd->size) {
		...
		pd->id_tbl =
		    (int32_t *) kmalloc(sizeof(int32_t) * pd->size * 2,
					GFP_KERNEL);
		if (NULL == pd->id_tbl) {
			...
			memcpy(pd->id_tbl, temp_id_tbl,
			       sizeof(int32_t) * pd->size);
			kfree(temp_id_tbl);
			pd->size *= 2;
			pd->id_tbl[pd->index++] = id;
		}
...
********************************************************************************/
static int32_t hwtc_alloc(struct hwtc_table_t *tbl,
			  struct hwtc_info_t *info,
			  char **pname, struct hwtc_private_data_t *pd)
{
	int32_t id = -1;
	int32_t index;
	...
	if (tbl->index < MAX_ENTRY_COUNT) {
		*pname = tbl->entry[tbl->index].name[0];
		...
		if (MAX_ENTRY_COUNT == index) {
			hwlog_warn
			    ("%s failed to allocate entry, table index(%d).\n",
			     __func__, tbl->index);
		} else {
			if (NULL != pd) {
				hwtc_record_id(id, pd);
...
Return:         -1 : error, 0 : ok
********************************************************************************/
static int32_t hwtc_setup(struct hwtc_table_t *tbl,
			  struct hwtc_info_t *info, char **pname)
{
...
static bool hwtc_dmd_report_judge(struct hwtc_entry_t *entry)
{
	...
	i = (entry->time_index + MAX_TIME_RECORD_COUNT -
	     1) % MAX_TIME_RECORD_COUNT;
	for (; i != entry->time_index;
	     i = ((i + MAX_TIME_RECORD_COUNT - 1) % MAX_TIME_RECORD_COUNT)) {
		pre_time = &entry->time[i];
		if (0 != pre_time->dmd_reported) {
			time_delta =
			    (cur_time->time[1].tv_sec * USEC_PER_SEC +
			     cur_time->time[1].tv_usec)
			    - (pre_time->time[1].tv_sec * USEC_PER_SEC +
			       pre_time->time[1].tv_usec);
...
static bool hwtc_dmd_data_handle(struct hwtc_entry_t *entry, int32_t output)
{
	...
	if (dmd_block_ring.block_count >= MAX_DMD_BLOCK_COUNT) {
		...
		strncpy(global_dmd_block[index].module_name,
			map_table[tag - 1].name,
		...
		memcpy(global_dmd_block[index].time, entry->time,
		       sizeof(entry->time));
		dmd_block_ring.write_p =
		    (dmd_block_ring.write_p + 1) % MAX_DMD_BLOCK_COUNT;
		dmd_block_ring.block_count++;
		hwlog_info("%s cout = %d, write_p = %d\n", __func__,
			   dmd_block_ring.block_count, dmd_block_ring.write_p);
	}
...
Return:         -1 : error, 0 : ok
********************************************************************************/
static int32_t hwtc_mark(struct hwtc_table_t *tbl, int32_t id, int32_t output)
{
	...
	do_gettimeofday(&entry->time[time_index].time[HWTC_BE(id)]);

#if HWTC_DEBUG
	hwlog_info("%s name:%s,id:0x%x, time:%ld.%ld.\n",
		   __func__, tbl->entry[HWTC_INDEX(id)].name, id,
		   tbl->entry[HWTC_INDEX(id)].time[time_index].
		   time[HWTC_BE(id)].tv_sec,
		   tbl->entry[HWTC_INDEX(id)].time[time_index].
		   time[HWTC_BE(id)].tv_usec);
#endif

	if (HWTC_BE(id)) {
		time_delta =
		    (entry->time[time_index].time[1].tv_sec * USEC_PER_SEC +
	   ...
	   size_t cnt, loff_t *ppos)
{
	struct hwtc_private_data_t *pd =
	    (struct hwtc_private_data_t *)filp->private_data;
	struct hwtc_array *tr = (struct hwtc_array *)pd->private;
	...
	if (HWTC_INVALID_ID == HWTC_INDEX(info.id)) {
		...
		if (0 == (0x01 & (entry->use >> HWTC_BE(info.id)))) {
			...
			if (HWTC_BE(info.id)) {
				info.output =
				    simple_strtoul(tendp + 1, NULL, 0);
			} else {
				info.threshold =
				    simple_strtoul(tendp + 1, NULL, 0);
			}
			info.id =
			    hwtc_setup(&tr->table[HWTC_TABLE(info.id)], &info,
				       &pname);
		} else {
			info.tag = simple_strtoul(iendp + 1, &tendp, 0);
...
static int hwtc_dump_entry(long fd, struct hwtc_entry_t *entry)
{
	...
	k = (entry->time_index + MAX_TIME_RECORD_COUNT -
	     1) % MAX_TIME_RECORD_COUNT;
	for (i = 0; i < MAX_TIME_RECORD_COUNT; i++) {
		time = &entry->time[k];
		if (0 != time->time_delta) {
			len =
			    snprintf(buffer, DUMP_BUFFER_LEN,
				     "B:%ld.%06ld s, E:%ld.%06ld s, Delta:%u us, Output:%d.\n",
				     time->time[0].tv_sec,
				     time->time[0].tv_usec,
				     time->time[1].tv_sec,
				     time->time[1].tv_usec, time->time_delta,
				     entry->output[k]);
			sys_write(fd, buffer, len);
		}
	...
	if (entry->time_index != 0) {
		...
		len =
		    snprintf(buffer, DUMP_BUFFER_LEN,
			     "COUNT:%u, AVG:%u, VAR:%u, MAX:%u, MIN:%u.\n",
			     pstatistic->count, pstatistic->average,
			     pstatistic->variance, pstatistic->max,
			     pstatistic->min);
		sys_write(fd, buffer, len);
	}

	s = (entry->statistic_index + MAX_STATISTIC_RECORD_COUNT -
	     1) % MAX_STATISTIC_RECORD_COUNT;
...
static int hwtc_dump(struct hwtc_array *tr, uint32_t tag, uint32_t code)
{
	...
	snprintf(file_name, FILE_NAME_LEN, "%s%s.%d", exce_dir, HWTC_LOG_NAME,
		 ((tag << FILE_NAME_TAG_OFFSET) | code));
	...
	for (i = 0; i < HWTC_CTBL_MAX; i++) {
		...
		for (j = 0; j < MAX_ENTRY_COUNT; j++) {
			if ((HWTC_MAIN_TAG(ptbl->entry[j].tag) ==
			     HWTC_MAIN_TAG(tag)) || (ALL_TAG == tag)) {
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (dmd_block_ring.block_count > 0) {
		memcpy(buf, &global_dmd_block[dmd_block_ring.read_p],
		       sizeof(struct hwtc_dmd_block_t));
		ret = sizeof(struct hwtc_dmd_block_t);
		dmd_block_ring.block_count--;
		dmd_block_ring.read_p =
		    (dmd_block_ring.read_p + 1) % MAX_DMD_BLOCK_COUNT;
		hwlog_info("%s cout = %d, read_p = %d\n", __func__,
			   dmd_block_ring.block_count, dmd_block_ring.read_p);
	}
	mutex_unlock(&dmd_block_ring.lock);
				 ...
				 const char *ubuf, size_t cnt)
{
	...
	if ((sys_access(BETA_OVERSEA_SYSTEM, O_RDONLY) < 0) &&
		...
		if ((sys_access(BETA_DOMESTIC_SYSTEM, O_RDONLY) == 0) ||
			...
			hwlog_info
			    ("%s this version is domestic beta version.\n",
			     __func__);
			hwtc_is_enable = true;
		}
	}
...
********************************************************************************/
static int32_t
hwtc_record(uint32_t tag, const char *name, int32_t id, int32_t input)
{
...
void
hwtc_begin(uint32_t tag, const char *name, int32_t *id, uint32_t threshold)
{
	...
	if (hwtc_is_enable) {
		HWTC_SET_BEGIN(*id);
		snprintf(buf, MAX_NAME_SIZE, "0x%x|0x%x|%d|pid(%ld)|%s", *id,
			 tag, threshold, sys_getpid(), name);
...
void hwtc_end(uint32_t tag, const char *name, int32_t *id, int32_t *output)
{
	...
	if (hwtc_is_enable) {
		HWTC_SET_END(*id);
		snprintf(buf, MAX_NAME_SIZE, "0x%x|0x%x|0x%x|pid(%ld)|%s", *id,
			 tag, *output, sys_getpid(), name);
