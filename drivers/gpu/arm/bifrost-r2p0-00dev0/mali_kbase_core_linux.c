...
copy_failed:
	...
	case KBASE_FUNC_MAP_IOVA:
	{
		int result;
		struct kbase_uk_mem_iova *info = args;
		if (sizeof(*info) != args_size)
			goto bad_size;

		result = kbase_mem_map_iova(kctx, info->gpu_addr, &(info->iova));
		if (result != 0)
		{
			info->header.ret = MALI_ERROR_FUNCTION_FAILED;
		}
		else
		{
			info->header.ret = MALI_ERROR_NONE;
		}

		break;
	}
	case KBASE_FUNC_UMAP_IOVA:
	{
		int result;
		struct kbase_uk_mem_iova *info = args;
		if (sizeof(*info) != args_size)
		...
		result = kbase_mem_umap_iova(kctx, info->gpu_addr);
		if (result != 0)
		{
			info->header.ret = MALI_ERROR_FUNCTION_FAILED;
		}
