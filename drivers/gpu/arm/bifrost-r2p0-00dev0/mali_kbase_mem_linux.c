...
static int kbase_check_buffer_size(struct kbase_context *kctx, u64 size)
{
	...
	return ((size == (size & ~((u64)cache_line_align - 1))) ?
			0 : (-EINVAL));
}
					...
					void __user *ptr)
{
	...
	return (((uintptr_t)ptr == ((uintptr_t)ptr & ~((u64)cache_line_align - 1))) ?
			0 : (-EINVAL));
}
...
int kbase_mem_map_iova(struct kbase_context *kctx, u64 gpu_addr, u32* iova)
{
	struct kbase_va_region *reg;
	int ret = -EINVAL;
	phys_addr_t *pageArray;
	size_t pageCount;
	...
	reg = kbase_region_tracker_find_region_base_address(kctx, gpu_addr);
	if (!reg || (reg->flags & KBASE_REG_FREE)) {
		pr_err("kbase_mem_map_iova called with nonexistent gpu_addr 0x%llX",
			gpu_addr);
		goto out_unlock;
	}
	...
	pageArray = kbase_get_cpu_phy_pages(reg);
	if (!pageArray)
		goto out_unlock;

	pageCount = kbase_reg_current_backed_size(reg);
	if (sg_alloc_table(table, (u32)pageCount, GFP_KERNEL))
		goto out_unlock;

	sg = table->sgl;
	for (; i < pageCount; i++){
		sg_set_page(sg, phys_to_page(pageArray[i]), (u32)PAGE_SIZE, 0);
	...
	ret = hisi_iommu_map_domain(sg, &format);
	if (ret == 0 ) {
		reg->iova_start = (u32)format.iova_start;
		reg->iova_size = (u32)format.iova_size;
		*iova = (u32)format.iova_start;
	}
...
int kbase_mem_umap_iova(struct kbase_context *kctx, u64 gpu_addr)
{
	struct kbase_va_region *reg;
	int ret = -EINVAL;
	struct iommu_map_format format;
	...
	reg = kbase_region_tracker_find_region_base_address(kctx, gpu_addr);
	if (!reg || (reg->flags & KBASE_REG_FREE)) {
		dev_warn(kctx->kbdev->dev, "kbase_mem_umap_iova called with nonexistent gpu_addr 0x%llX",
			gpu_addr);
		goto out_unlock;
	}

	memset(&format, 0 ,sizeof(format));
	format.iova_start = reg->iova_start;
	format.iova_size = reg->iova_size;
