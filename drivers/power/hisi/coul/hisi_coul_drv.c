...
/****************************************************************************
  Function:     hisi_battery_aging_safe_level
  Description:  get the battery aging safe level
  Input:        void
  Output:       NA
  Return:       aging safe level
****************************************************************************/
BASP_LEVEL_TYPE hisi_battery_aging_safe_level(void)
{
    /*declare the local variable of struct hisi_coul_ops */
    LOCAL_HISI_COUL_OPS();

    /*execute the operation of coul module */
    if (ops && ops->aging_safe_level)
    {
        return ops->aging_safe_level();
    }
    return -EPERM;
}
