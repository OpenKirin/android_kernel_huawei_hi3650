...
typedef struct
{
    ...
    unsigned int raw_pattern;//bayer_pattern_e
    unsigned int mono_mode[2];
    int          reserved[11];
} common_info_md_t;
...
typedef struct
{
    ...
    short IVtop;
    short IVbottom;
} GKMCrop_hw_cfg_struct;

typedef struct
{
    DIS_hw_delta_struct  dis_delta;
    GKMCrop_hw_cfg_struct gkmCrop_config;
    int                      reserved[10];
} stat_dis_t;
...
typedef struct
{
    ...
    MiniLSC_reg_hw_struct minilsc_reg_hw;
    int reserved[64];
} lsc_out_param_t;
...
typedef struct
{
    ...
    af_msg                  msg;
    unsigned int            af_ctrl_cur_code[2];
    unsigned int            af_ctrl_cur_virtual_code[2];
    signed long long        af_ctrl_focus_done_time[2];
    unsigned int            vcm_stable[2];
    unsigned int            hw_ipp_vcm_stable[2];
    int                     current_distance[2];
    af_md_detailed_info_t   af_detailed_info[2];
    af_debug_info           debug_info[2];
    af_display_info_t       display_info;
    unsigned long long int  contrast_value[2];
    int                     pointFlag[2];
    int                     af_mmi_report;
    int                     af_mmi_result;
    int                     af_mmi_index;
...
typedef struct
{
    CC_HWReg_Dynamic_Param_struct cc_dynamic_reg_hw;
...
typedef struct
{
   Gamma_dynamic_out_struct gamma_dynamic_out[PIPE_MAX];
   tone_map_t tone_map;
   char reserved[16];
} gamma_out_param_t;
...
typedef struct
{
    CE_HWReg_Dynamic_Param_struct ce_dynamic_reg_hw;
...
typedef struct
{
    ...
    blc_md_t                blc_md[2];
    stat3a_md_t             stat_3a[2];
    stat_dis_t              stat_dis;
    dgamma_info_t           dgamma_md[2];
    lsc_out_param_t         lsc_md[2];
    dpcc_info_t             dpcc_md[2];
    rawnf_md_t              rawnf_md[2];
    exposure_param_struct   ae_md;
    af_md_t                 af_md;
    awb_out_param_t         awb_md[2];
    flash_md_t              flash_md;
    face_pos_md_t           face_pos;
    gcd_info_t              gcd_md[2];
    cc_out_param_t          cc_md[2];
    drc_out_param_t         drc_md[2];
    gamma_out_param_t       gamma_md[2];
    ce_out_param_t          ce_md[2];
    tnr_md_t                tnr_md;
    yuvnf_md_t              yuvnf_md[2];
    sharpen_md_t            sharpen_md[2];
    ois_result_t            ois_result;
    laser_info_md_t         laser_md;
    monitor_t               monitor_result[2];
    // dmap_md_t               dmap_md;
    altek_md_t              altek_md;
    other_md_t              other_md;
