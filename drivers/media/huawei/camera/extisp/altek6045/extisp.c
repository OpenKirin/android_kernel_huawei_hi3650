...
static void misp_set_firmware_path(char * path,int type)
{
    if(type == EXTISP_AL6010){
        snprintf(mini_isp_boot_code,NAME_MAX,"%s%s",path,MINI_ISP_BOOT_CODE_AL6010);/* [false alarm]:code not used */
        snprintf(mini_isp_basic_code,NAME_MAX,"%s%s",path,MINI_ISP_BASIC_CODE_AL6010);/* [false alarm]:code not used */
    } else if (type == EXTISP_AL6045) {
        snprintf(mini_isp_boot_code,NAME_MAX,"%s%s",path,MINI_ISP_BOOT_CODE_AL6045);/* [false alarm]:code not used */
        snprintf(mini_isp_basic_code,NAME_MAX,"%s%s",path,MINI_ISP_BASIC_CODE_AL6045);/* [false alarm]:code not used */
    }
...
static int misp_probe(struct spi_device *spi)
{
	...
	cam_info("probe - tx_buf=0x%pK rx_buf=0x%pK ext_buf=0x%pK, irq_buf=0x%pK",
