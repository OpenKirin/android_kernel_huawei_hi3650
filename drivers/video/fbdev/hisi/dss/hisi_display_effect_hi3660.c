...
int hisifb_ce_service_set_lut(struct fb_info *info, void __user *argp)
{
	...
	if (hiace_set_interface.result == 0) {
		...
		bl_ctrl->ctrl_bl_delta = (bl_enable_ctrl->ctrl_bl_enable == 1 && g_enable_effect & ENABLE_EFFECT_BL && pinfo->hiace_param.iDoAPLC == 1)
			?  hiace_set_interface.backlight : 0;
...
ssize_t hisifb_display_effect_ce_ctrl_store(struct fb_info *info, const char *buf, size_t count)
{
	...
	EFFECT_DEBUG_LOG(DEBUG_EFFECT_ENTRY, "[effect] ce_mode change to %d\n", ce_ctrl->ctrl_ce_mode);
...
bool hisifb_display_effect_is_need_ace(struct hisi_fb_data_type *hisifd)
{
	...
	EFFECT_DEBUG_LOG(DEBUG_EFFECT_ENTRY, "[effect] g_enable_effect:%d, ce_mode:%d, sre_enable:%d, to_stop_hdr:%d, to_stop_sre:%d\n", g_enable_effect, hisifd->ce_ctrl.ctrl_ce_mode, hisifd->sre_ctrl.ctrl_sre_enable, hisifd->hiace_info.to_stop_hdr, hisifd->hiace_info.to_stop_sre);
...
bool hisifb_display_effect_fine_tune_backlight(struct hisi_fb_data_type *hisifd, int backlight_in, int *backlight_out)
{
	...
	if (hisifd->bl_level > 0) {
		if (hisifd->bl_enable_ctrl.ctrl_bl_enable == 1 && g_enable_effect & ENABLE_EFFECT_BL && pinfo->hiace_param.iDoAPLC != 0) {
