...
static int init_cc_resources(struct platform_device *plat_dev)
{
	...
	new_drvdata->ccp_clk = devm_clk_get(&plat_dev->dev, "clk_secp");
	if (IS_ERR(new_drvdata->ccp_clk)) {
		DX_LOG_ERR("clk_get: secp clk not found!\n");
		rc = -ENODEV;
		goto init_cc_res_err;
	}

	rc = clk_prepare_enable(new_drvdata->ccp_clk);
	if (0 != rc) {
		DX_LOG_ERR("clk_get: secp clk enable failed!\n");
		goto init_cc_res_err;
	}
...
init_cc_res_err:
	...
	if (new_drvdata != NULL) {
		...
		if (req_mem_cc_regs != NULL) {
				if (irq_registered) {
#ifndef CONFIG_CRYPTO_SECP_POWER_CTRL
	                 if(!IS_ERR(new_drvdata->ccp_regu)){
